...
func requiredRenderingFormatsDescribeExactCrossApiLayouts() {
  ...
  #expect(formats.map(\.androidFormat) == [2, 1, 22, 43])
  #expect(formats.map(\.bytesPerPixel) == [4, 4, 8, 4])
  #expect(
    formats.map(\.componentBits) == [
      [8, 8, 8, 0],
      [8, 8, 8, 8],
      [16, 16, 16, 16],
      [10, 10, 10, 2],
    ])
  #expect(formats.map(\.vulkanFormat) == [37, 37, 97, 64])

...
@Test func androidAddonPackagingProducesASignedCompleteArtifact() async throws {
    ...
    let context = WorkspaceContext(root: temporary, environment: ["HOME": temporary.path])
    ...
    try await AndroidAddonPackageCommand(context: context).run(
        runtimeRoot: runtime,
        aospGeneration: aosp,
        compatibilityURL: compatibilityURL,
        aospSigningKey: aospKey,
        addonSigningKey: addonKey,
        output: output)

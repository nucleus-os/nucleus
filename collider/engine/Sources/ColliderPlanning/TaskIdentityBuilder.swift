    ...
    func identity(
        ...
        for input in identityInputs(of: task) {
            ...
            case .tree(let path):
                ...
                encoder.append(tag: 17, bytes: try services.digestTree(path).bytes)
            case .optionalTree(let path, let fallback):
                encoder.append(tag: 72, string: path.string)
                if let digest = try services.optionalTreeDigest(path) {
        ...
        try encode(action: task.action, into: &encoder, services: services)

...
@Test func domainSourcePreparationsMaterializeTheirDeclaredReferences()
    async throws
{
    ...
    let swiftCheckout = fixture.appendingPathComponent("swift")
    ...
    let swiftTask = TaskDeclaration(
        id: TaskID(rawValue: "fixture.swift-source"),
        component: ComponentID(rawValue: "fixture"),
        inputs: [.value(name: "reference", bytes: Array("main".utf8))],
        outputs: [
            OutputDeclaration(
                path: FilePath(
                    swiftCheckout.appendingPathComponent("marker.txt").path),
                validation: .regularFile),
        ],
        operation: .prepareSwiftSource(SwiftSourcePreparation(
            repository: FilePath(swiftCheckout.path),
            remote: remote.path,
    ...
    _ = try await runtime.execute(
        graph: TaskGraph([swiftTask, depotTask]),
        selected: [swiftTask.id, depotTask.id],
        stateRoot: FilePath(
    ...
    #expect(
        try String(
            contentsOf: swiftCheckout.appendingPathComponent("marker.txt"),

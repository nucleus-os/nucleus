...
struct ProfileCapture {
    ...
    func run(
        ...
        guard FileManager.default.isExecutableFile(atPath: receiver.path),
            FileManager.default.isExecutableFile(atPath: exporter.path)
        else {
    ...
    private func waitForSessionReady(
        ...
        let graphicalSession =
            environment["WAYLAND_DISPLAY"] != nil || environment["DISPLAY"] != nil
        let hint =
            graphicalSession
    ...
    private func writeMetadata(
        ...
        try Data((values.joined(separator: "\n") + "\n").utf8).write(
            to: directory.appendingPathComponent("metadata.txt"), options: .atomic)
    }
    ...
    private func exportAndSummarize(
        ...
        try Data(events.utf8).write(
            to: directory.appendingPathComponent("trace-events.csv"), options: .atomic)
        try Data(plots.utf8).write(
            to: directory.appendingPathComponent("trace-plots.csv"), options: .atomic)
        ...
        for row in events.split(separator: "\n").dropFirst() {
            ...
            if let name = fields.first, !name.isEmpty {
                ...
                if fields.count > 4, let duration = Int(fields[4]) {
                    for budget in [4_166_667, 2_777_778, 2_000_000] where duration > budget {
                        overBudget[key, default: [:]][budget, default: 0] += 1
                    }
                }
            }
        }
        ...
        let budgets = overBudget.flatMap { name, values in
            values.map { "\(name).over_\(budgetNames[$0.key]!).budget=\($0.value)" }
        }
        ...
        let summary =
            (["framebuffer_effect_cache.create_failures=0"]
            + eventCounts.sorted { $0.key < $1.key }.map { "\($0.key)=\($0.value)" }
            + numericPlotLines
            + budgets.sorted()).joined(separator: "\n") + "\n"
        try Data(summary.utf8).write(
            to: directory.appendingPathComponent("trace-summary.txt"), options: .atomic)

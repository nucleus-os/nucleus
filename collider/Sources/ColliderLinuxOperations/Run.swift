...
enum OptimizationMode: String, Equatable, ExpressibleByArgument {
    ...
    var runtimeOptimization: RuntimeOptimization {
        switch self {
        case .debug: .debug
...
struct RunOptions: ParsableArguments {
    ...
    var buildOptions: RuntimeBuildSelection {
        RuntimeBuildSelection(
            optimization: effectiveOptimization,
            tracy: tracy,
            sanitizer: sanitizer)
    }

    var effectiveOptimization: RuntimeOptimization {
        optimization?.runtimeOptimization ?? (tracy ? .release : .debug)
    }

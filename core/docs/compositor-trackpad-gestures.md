...
New file `PointerGestures.swift` in `NucleusCompositorWaylandRuntime`, following the router protocol pattern used by `CursorShape.swift` and `RelativePointer.swift` (a manager object holding a C request vtable, registered on the router). The `zwp_pointer_gestures_v1` server-side protocol C is generated by the `NucleusCompositorGenerateWaylandC` build plugin — add the already-vendored `third-party/wayland-protocols/unstable/pointer-gestures/pointer-gestures-unstable-v1.xml` to the plugin's protocol list so its `-server-protocol.h` / `-protocol.c` are emitted for the runtime to bind against.
...
- `compositor-core/Sources/NucleusCompositorGenerateWaylandC/plugin.swift` — Phase 3 (register the pointer-gestures protocol XML)

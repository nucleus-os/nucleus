...
This does not block `tools/collider run --present-mode mailbox_latest_wins`. Mailbox/latest-wins is allowed on Nvidia so current driver behavior can be measured directly. Treat the notes below as things to verify in logs and profiles, not as a reason to refuse the mode before running it.

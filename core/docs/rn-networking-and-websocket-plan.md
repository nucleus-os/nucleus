...
- The RN fixture continues to render as a Wayland client during a
  `collider run --tracy` session — no behavioral regression for
...
- `nm` on the final binary shows both BoringSSL symbol sets
  ...
  collision.
- Capture a `collider run --tracy` session to confirm the
...
- New: `bundles/shell/networking-smoke/index.jsx`. A bundle that
  ...
  - `new FormData(); fd.append("name", "value");
    ...
    response.
- Runs as an RN Wayland client inside a `collider run --tracy`
  session.
...
- `collider run --tracy --seconds 30` with the networking-smoke RN
...
- `swift test -Xswiftc -cxx-interoperability-mode=default
  --filter NucleusNetworkingTests --filter NucleusReactRuntimeCxxTests` green.
- `collider run --tracy --seconds 30` with the RN development

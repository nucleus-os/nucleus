...
package struct RawPixelBuffer: Equatable, Sendable {
    package var width: Int
    package var height: Int
    /// Bytes per row in `pixels`, which may exceed `width * bytesPerPixel`.
    package var rowStride: Int
    package var order: PixelChannelOrder
    ...
    /// be, so this defaults to the D-Bus contract rather than the GPU's.
    package var isPremultiplied: Bool
    ...
    package init(
        ...
        if let rowStride {
            ...
            // and diagnostics can inspect malformed sender input without trapping.
    ...
    /// itself consistently.
    package func normalizedRGBA() -> [UInt8]? {
    ...
    package func contentHash() -> UInt64 {

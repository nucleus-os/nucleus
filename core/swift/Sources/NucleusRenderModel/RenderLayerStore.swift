...
package enum ExtentClip: Equatable, Sendable {
    case none
    case rect(RenderRect)
    case empty
}
...
/// `appendUniqueLayerID`.
package func appendUniqueLayerID(_ list: inout [UInt64], _ layerId: UInt64) {
    if list.contains(layerId) { return }
    list.append(layerId)
}

// MARK: - RenderRect algebra

/// Translate a rect by an offset (size unchanged). Mirrors `offsetRect`.
package func offsetRect(_ rect: RenderRect, _ offset: Point2D) -> RenderRect {
    RenderRect(x: rect.x + offset.x, y: rect.y + offset.y, w: rect.w, h: rect.h)
}

/// Bounding union of two rects. Mirrors `unionRect`.
package func unionRect(_ a: RenderRect, _ b: RenderRect) -> RenderRect {
    let left = min(a.x, b.x)
    let top = min(a.y, b.y)
    let right = max(a.x + a.w, b.x + b.w)
    let bottom = max(a.y + a.h, b.y + b.h)
    return RenderRect(x: left, y: top, w: right - left, h: bottom - top)
}

/// True when the rect has strictly positive area. Mirrors `rectHasArea`.
package func rectHasArea(_ rect: RenderRect) -> Bool {
    rect.w > 0 && rect.h > 0
}

/// Intersection of two rects, or `nil` if they don't overlap. Mirrors
/// `intersectRect`.
package func intersectRect(_ a: RenderRect, _ b: RenderRect) -> RenderRect? {
...
/// `unionMaybeRect`.
package func unionMaybeRect(_ result: inout RenderRect?, _ rect: RenderRect) {
...
package func clipExtentRect(_ clip: ExtentClip, _ rect: RenderRect) -> RenderRect? {
...
/// `lerpRect`.
package func lerpRect(_ from: RenderRect, _ to: RenderRect, _ progress: Float) -> RenderRect {
...
package func accumulateExtentClip(_ parentClip: ExtentClip, localClip: RenderRect?) -> ExtentClip {
    switch parentClip {
    case .empty:
        return .empty
    case .none:

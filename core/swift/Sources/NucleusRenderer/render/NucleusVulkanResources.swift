...
@unsafe private func withRequiredMutableFeatureChain<R>(
    ...
    unsafe v12.timelineSemaphore =
        enableRequiredFeatures && contract.requiresTimelineSemaphore ? 1 : 0
...
@unsafe package func withRequiredFeatureQueryChain<R>(
    contract: VkRequirements.Contract,
    _ body: (UnsafeMutablePointer<VkPhysicalDeviceFeatures2>) -> R
) -> R {
...
@unsafe package func withRequiredFeatureEnableChain<R>(
    contract: VkRequirements.Contract,
...
@unsafe private func supportsExtensions(
    _ required: [String],
    enumerate: (
        _ count: UnsafeMutablePointer<UInt32>, _ out: UnsafeMutablePointer<VkExtensionProperties>?
    ) -> VkResult
) -> Bool {
    ...
    unsafe withRequiredFeatureQueryChain(contract: contract) { pointer in
        ...
        while let raw = unsafe feature {
            ...
            case VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_VULKAN_1_3_FEATURES:
                synchronization2 =
                    unsafe raw.assumingMemoryBound(
            ...
            case VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_VULKAN_1_2_FEATURES:
                timeline =
                    ...
                    ).pointee.timelineSemaphore != 0
            case VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_VULKAN_1_1_FEATURES:
                ycbcr =
                    unsafe raw.assumingMemoryBound(
                        to: VkPhysicalDeviceVulkan11Features.self
                    ).pointee.samplerYcbcrConversion != 0
...
@safe package struct InstanceOwner: ~Copyable {
    package let handle: VkInstance
    package let dispatch: VK.InstanceDispatch

    @unsafe package init(adopting handle: VkInstance, dispatch: VK.InstanceDispatch) {
    ...
    @unsafe package static func create(
        ...
        guard unsafe enumerateVersion(&loaderVersion) == VK_SUCCESS,
            ...
            unsafe supportsExtensions(
                contract.instanceExtensions,
                enumerate: { count, out in
            ...
            unsafe withCStringArray(contract.instanceExtensions) { extPtr, extCount in
                unsafe withCStringArray(layers) { layerPtr, layerCount in
                    withUnsafePointer(to: &app) { appPtr in
                        ...
                        if unsafe createFn(&ci, nil, &inst) == VK_SUCCESS {
                            unsafe created = unsafe inst
                        }
                    }
                }
            }
...
@safe package struct DeviceOwner: ~Copyable {
    package let handle: VkDevice
    package let dispatch: VK.DeviceDispatch

    @unsafe package init(adopting handle: VkDevice, dispatch: VK.DeviceDispatch) {
        unsafe self.handle = unsafe handle
        self.dispatch = dispatch
    }
    ...
    @unsafe package func queue(family: UInt32, index: UInt32 = 0) -> VkQueue? {
    ...
    @safe package struct PhysicalSelection {
        package var physicalDevice: VkPhysicalDevice
        package var graphicsQueueFamily: UInt32
    }
    ...
    @unsafe package static func selectPhysicalDevice(
        instance: VkInstance,
        dispatch: VK.InstanceDispatch,
        contract: VkRequirements.Contract,
        ...
        guard
            let devices = unsafe VkEnumerate.array({ count, out in
        ...
        for index in unsafe devices.indices {
            ...
            guard VkVersion(raw: properties.apiVersion) >= contract.minimumApiVersion,
                ...
                unsafe supportsFeatures(
                    physicalDevice: device, dispatch: dispatch, contract: contract)
            else { continue }
            var count: UInt32 = 0
            ...
            for (index, family) in families.enumerated() {
                ...
                if let requiredPresentationSurface = unsafe requiredPresentationSurface {
                    guard
                        let getSurfaceSupport = unsafe dispatch.vkGetPhysicalDeviceSurfaceSupportKHR
                    else {
                        continue
                    }
                    var supported: VkBool32 = 0
                    guard
                        unsafe getSurfaceSupport(
                            device, familyIndex, requiredPresentationSurface, &supported)
                            == VK_SUCCESS,
                        supported != 0
                    else { continue }
                }
                ...
                return unsafe PhysicalSelection(
                    physicalDevice: device, graphicsQueueFamily: familyIndex)
            }
        }
        return nil
    }
            ...
            withUnsafePointer(to: &queueInfo) { queuePtr in
                unsafe withRequiredFeatureEnableChain(
                    ...
                    unsafe withCStringArray(contract.deviceExtensions) { extPtr, extCount in
                        ...
                        if unsafe createFn(selection.physicalDevice, &ci, nil, &device)
                            == VK_SUCCESS
                        {

...
extension ViewLayerPublisher {
    ...
    func publishPaint(
        ...
        guard
            state.paintWidth != width || state.paintHeight != height
            ...
            })
        {
            ...
            cacheDelta.paintInsertions[cacheKey, default: []].append(
                PaintCacheEntry(
                    recording: recording,
        ...
        stagePaintReferenceRemoval(
            for: state,
            cacheDelta: &cacheDelta)
        cacheDelta.paintReferenceChanges.append(
            PaintReferenceChange(
                key: cacheKey,
                recording: recording,
                delta: 1))
        ...
        didMutate = true
        metrics.paintBytes &+=
            UInt64(recording.payload.count)
    ...
    func paintCacheKey(
        ...
        for command in recording.commands {
            ...
            hasher.combine(command.shading)

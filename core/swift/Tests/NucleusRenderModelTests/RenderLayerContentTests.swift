...
@Suite struct RenderLayerContentTests {
    @Test func renderLayerContent() {
        ...
        #expect(
            LayerContent.paint(PaintContentHandle(raw: 5)) == .paint(PaintContentHandle(raw: 5)),
            "content-paint-equal")
        #expect(
            LayerContent.paint(PaintContentHandle(raw: 5)) != .paint(PaintContentHandle(raw: 6)),
            "content-paint-differs")
        #expect(
            LayerContent.external(IOSurfaceID(raw: 1)) != .snapshot(SnapshotHandle(raw: 1)),
            "content-case-differs")

        // InitialContent lowers to the matching LayerContent.
        #expect(
            InitialContent.snapshot(SnapshotHandle(raw: 9)).resolved()
                == .snapshot(SnapshotHandle(raw: 9)),
            "initial-resolves")
        ...
        #expect(
            ContentDelta.external(IOSurfaceID(raw: 4)).apply(to: base)
                == .external(IOSurfaceID(raw: 4)),
        ...
        #expect(
            EffectShape.rrect(rect: (0, 0, 4, 4), radii: (1, 1, 1, 1))
                == .rrect(rect: (0, 0, 4, 4), radii: (1, 1, 1, 1)), "shape-rrect-equal")
        #expect(
            EffectShape.rect((0, 0, 4, 4)) != .rrect(rect: (0, 0, 4, 4), radii: (0, 0, 0, 0)),
            "shape-case-differs")
        ...
        #expect(
            attach.tint == .init() && attach.opacity == 1 && attach.groupId == 0,
            "attach-defaults")
        var attach2 = attach
        ...
        #expect(
            LayerKind.remoteHost(ContextID(raw: 7)) == .remoteHost(ContextID(raw: 7)),
            "kind-remote-equal")
        #expect(
            LayerKind.remoteHost(ContextID(raw: 7)) != .remoteHost(ContextID(raw: 8)),
            "kind-remote-differs")
        let bk = BackdropKindParams(shape: .rect((0, 0, 1, 1)))
        #expect(
            bk.materialRole == .default && bk.appearance == .auto && bk.state == .active

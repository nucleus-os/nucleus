...
@safe struct AndroidAssetProvider {
    ...
    func smokeValue(path: UnsafePointer<CChar>) throws -> Int32 {
        if unsafe !isNormalizedAssetPath(path) {
            throw AssetReadError.pathRejected
        }

        guard let asset = unsafe nucleus_android_asset_open(
            manager,
            path,
            AASSET_MODE_BUFFER)
        else {
        ...
        let length = unsafe nucleus_android_asset_get_length64(asset)
...
private func isNormalizedAssetPath(_ path: UnsafePointer<CChar>) -> Bool {
    ...
    var len = 0
    while unsafe path[len] != 0 { len += 1 }

    if len == 0 || UInt8(bitPattern: unsafe path[0]) == slash { return false }

    var segmentStart = 0
    var i = 0
    while i <= len {
        if i == len || UInt8(bitPattern: unsafe path[i]) == slash {
            let segLen = i - segmentStart
            if segLen == 0 { return false }
            if segLen == 1
                && UInt8(bitPattern: unsafe path[segmentStart]) == dot
            {
                return false
            }
            if segLen == 2
                && UInt8(bitPattern: unsafe path[segmentStart]) == dot
                && UInt8(bitPattern: unsafe path[segmentStart + 1]) == dot
            {
                return false
            }
            segmentStart = i + 1

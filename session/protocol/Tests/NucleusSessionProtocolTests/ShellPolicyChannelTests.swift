...
@Suite struct ShellPolicyChannelTests {
    ...
    @Test func policyChannelRejectsAnotherProtocolVersion() throws {
        ...
        let shell = ShellPolicyChannel(
            owning: shellTransport.takeFileDescriptor())
        try server.send(ShellPolicyPublication(
    ...
    @Test func attachmentTransfersExactlyOneOwnedPolicyEndpoint() throws {
        ...
        #expect(unsafe Glibc.socketpair(
            AF_UNIX, Int32(SOCK_SEQPACKET.rawValue), 0, &policy) == 0)
        defer {
            for descriptor in policy where descriptor >= 0 {
                _ = Glibc.close(descriptor)
            }
        }

        try sender.send(policyDescriptor: policy[0])
        let attachment = try receiver.receive()

        #expect(attachment.rawValue >= 0)
        #expect(Glibc.fcntl(attachment.rawValue, F_GETFD) >= 0)
    }

    @Test func inheritedDescriptorArgumentsRequireOneValidValue() throws {
        #expect(try ShellPolicyAttachmentChannel.inherited(
            arguments: ["shell"]) == nil)

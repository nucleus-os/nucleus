...
[[ -e "/Library/LaunchDaemons/$runner_service_label.plist" ]] \
  && service_presence=present
local_state="$(nucleus_handoff_local_state "$account_presence" "$service_presence")"
...
case "$handoff_action" in
  provision)
    ...
    registration_token="$(gh api --method POST \
      "repos/$organization/$repository_name/actions/runners/registration-token" \
      --jq .token)"
...
for attempt in {1..60}; do
  registered_runner_name="$(gh api "repos/$organization/$repository_name/actions/runners" \
    --jq ".runners[] | select(.name == \"$runner_name\") | .name")"
  runner_status="$(gh api "repos/$organization/$repository_name/actions/runners" \
...
runner_labels="$(gh api "repos/$organization/$repository_name/actions/runners" \

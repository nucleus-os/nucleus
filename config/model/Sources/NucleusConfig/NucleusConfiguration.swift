...
public struct NucleusConfiguration: Codable, Equatable, Sendable {
    ...
    public var shell: ShellPreferences
    ...
    public init(
        ...
        shell: ShellPreferences = .defaults
    ) {
    ...
    /// Canonical shape used by configuration decoders to audit unknown keys.
    /// Array fields contain one representative element so their element schema
    /// remains discoverable without granting the model filesystem authority.
    public static let schemaReference = NucleusConfiguration(
        configVersion: currentVersion,
        input: .defaults,
        binds: DefaultBinds.table,
        outputs: [OutputConfig(
    ...
    public func applying(_ part: NucleusConfigurationPart) -> NucleusConfiguration {
        NucleusConfiguration(
            ...
            shell: shell.applying(
                part.shell ?? ShellPreferencesPart()))
    }
}
...
public struct NucleusConfigurationPart: Decodable, Equatable, Sendable {
    ...
    public var binds: [KeyBind]?
    public var outputs: [OutputConfig]?
    public var shell: ShellPreferencesPart?

    ...
    for proto in closure {
        for iface in proto.interfaces {
            for (opcode, request) in iface.requests.enumerated() {
                ...
                let childInterface = typedNewID
                    .flatMap(\.interface)
                    .map { "&\($0)_interface" }
                    ?? "NULL"
                let flags = request.isDestructor
                ...
                let suffix = wireArguments.isEmpty
                    ? ""
...
func clientRequestCArgument(
    ...
    let name = esc(argument.name)
    if let _ = resolvedEnumerationName(argument, in: interface) {
...
func clientRequestDeclaration(
    ...
    let returnType = createdArgument.map {
    ...
    let callArguments = (["_proxy"] + arguments.map {

...
package enum SwiftWaylandGenerator {
    package static func run(arguments: [String]) throws {
        ...
        var out = """
             ...
             *
             * Façades over libwayland's own \(mode.rawValue) headers for declarations the Swift clang
             * importer cannot express directly. This header adds interface-descriptor accessors,
             * wl_fixed helpers, and first-party \(mode == .server ? "request" : "event") tables whose
        ...
        func callbackFieldName(_ name: String) -> String {
            cxxKeywordIdentifiers.contains(name)
                ? "swift_wayland_wl_kw_\(name)" : name
        }

        func serverRequestParameters(_ request: WMsg) -> [String] {
            ...
            for argument in request.args {
                ...
                default:
                    fatalError(
                        "unsupported request argument type '\(argument.type)' in generated callback"
                    )
                }
            }
            return parameters
        ...
        func clientEventParameters(_ event: WMsg) -> [String] {
            var parameters = ["void *", "struct wl_proxy *"]
            for argument in event.args {
                ...
                case "array":
                    parameters.append("struct wl_array *")
                case "object", "new_id":
                    parameters.append("struct wl_proxy *")
                default:
                    fatalError(
                        "unsupported event argument type '\(argument.type)' in generated callback"
                    )
                }
            }
            return parameters
        ...
        if mode == .server {
            ...
            for proto in closure {
                for iface in proto.interfaces where iface.requestCount > 0 {
                    ...
                    if proto.name != "wayland" {
                        out +=
                            "_Static_assert(sizeof(swift_wayland_\(iface.name)_requests) == sizeof(struct \(iface.name)_interface), \"\(iface.name) request layout mismatch\");\n"
                    }
                }
            }
        } else {
            ...
            for proto in closure {
                for iface in proto.interfaces where !iface.events.isEmpty {
                    out += "struct swift_wayland_\(iface.name)_events {\n"
                    ...
                    out += "};\n"
                    out +=
                        "_Static_assert(sizeof(struct swift_wayland_\(iface.name)_events) == sizeof(struct \(iface.name)_listener), \"\(iface.name) listener layout mismatch\");\n"
                }
            }
        }
                ...
                for iface in proto.interfaces {
                    ...
                    source.add(
                        clientDeclaration([
                            ...
                            "            unsafe swift_wayland_client_install_listener(proxy, \(P)Client.listener, data)",
                            "        }",
                            "    }",
                            "}",
                        ]))

...
private extension WaylandMessage {
    var args: [WaylandArgument] {
        get { arguments }
        set { arguments = newValue }
    }
}

private extension WaylandProtocol {
...
let enumerationsByName: [String: (interface: Iface, enumeration: WaylandEnumeration)] =
    Dictionary(uniqueKeysWithValues: closure.flatMap { proto in
                ...
                for request in iface.requests {
                    ...
                    for argument in request.args {
                        ...
                        case "new_id" where argument.interface == nil:
                            parameters.append(contentsOf: ["const char *", "uint32_t", "uint32_t"])
                        case "new_id":
                            parameters.append("uint32_t")
                        default:
                            fatalError("unsupported core request argument type '\(argument.type)' in \(iface.name).\(request.name)")
                        }
                    }
                    out += "    void (*\(request.name))(\(parameters.joined(separator: ", ")));\n"
                }
...
func swiftParamType(_ a: WArg, in interface: Iface, isEvent: Bool = false) -> String {
    ...
    case "object":
        ...
        return "WaylandBorrowedObject<\(upperCamel(interface))Server>"
            + (a.allowNull ? "?" : "")
    case "string": return isEvent ? "UnsafePointer<CChar>?" : "String" + (a.allowNull ? "?" : "")
...
func requestArgumentExpression(_ argument: WArg, in interface: Iface) -> String {
    let raw = ["object", "string", "array"].contains(argument.type)
        ? "_request_\(argument.name)"
        : esc(argument.name)
    let label = esc(argument.name)
    switch argument.type {
    case "int" where argument.enumName != nil:
        return "\(label): \(resolvedEnumerationName(argument, in: interface)!)(rawValue: UInt32(bitPattern: \(esc(argument.name))))"
    case "uint" where argument.enumName != nil:
        return "\(label): \(resolvedEnumerationName(argument, in: interface)!)(rawValue: \(esc(argument.name)))"
    case "fixed":
    ...
    case "object":
        ...
        if argument.allowNull {
            return "\(label): \(raw) == nil ? nil : .some(WaylandBorrowedObject<\(type)Server>(\(raw)!))"
        }
            ...
            for enumeration in interface.enumerations {
                ...
                let declaration = StructDeclSyntax(
                    ...
                    for entry in enumeration.entries {
                        let numericValue = entry.value.hasPrefix("0x")
        ...
        for iface in proto.interfaces {
            ...
            func methodParams(_ r: WMsg) -> String {
                ...
                (["_ request: WaylandRequest<\(P)Server>"]
                    + r.args.map { "\(esc($0.name)): \(swiftParamType($0, in: iface))" }).joined(separator: ", ")
            }
            ...
            for e in emitsEvents ? iface.events : [] {
                ...
                var ca = ["target"]
                for a in e.args {
                    sp.append("\(esc(a.name)): \(swiftParamType(a, in: iface, isEvent: true))")
                    switch a.type {
            ...
            for r in dispatchesRequests ? iface.requests : [] {
                ...
                if isPureDestructor(r) {
                    ...
                    lines.append(contentsOf: actorBoundaryBindingLines(
                        for: r, includeClient: false).map { "        \($0)" })
                ...
                if hasNewId {
                    ...
                    for a in r.args {
                        ...
                        case "new_id":
                            let t = a.interface ?? ""
                            call.append("\(esc(a.name)): WlNewId<\(upperCamel(t))Server>(client: requestClient, id: \(esc(a.name)), version: Swift::min(wl_resource_get_version(requestResource), Int32(\(ifaceVersion[t] ?? 1))))")
                        default:
                            call.append(requestArgumentExpression(a, in: iface))
                        }
                    }
                    let args = (["WaylandRequest<\(P)Server>(requestResource)"] + call).joined(separator: ", ")
                    lines.append(
                        "    guard let res = unsafe res, let client = unsafe client, let h = unsafe handler(res) else { return }")
                    lines.append(contentsOf: actorBoundaryBindingLines(
                        for: r, includeClient: true).map { "    \($0)" })
                    lines.append(contentsOf: [
            ...
            if emitsEvents {
                ...
                for event in iface.events where event.since > 1 {
                    ...
                    eventMembers.append(declaration([
                        "var \(capability): Bool {",
                ...
                for event in iface.events {
                    ...
                    let genericParameters = arrayArguments.isEmpty
                        ? ""
                        : "<" + arrayArguments.map {
            ...
            let requiresPolicyOwner = dispatchesRequests
                && iface.requests.contains { !isPureDestructor($0) }
            let ownerConstraint = requiresPolicyOwner
                ? "\(P)Requests"
                : "AnyObject"
            var newIDMembers = [
                declaration([
            ...
            {
                newIDMembers.append(declaration([
                    "@discardableResult",
                    "@MainActor",
            ...
            let serverFactories = try ExtensionDeclSyntax(
                ...
                declaration([
                    "@discardableResult",
                    "@MainActor",
                    "static func createResource<Owner: \(ownerConstraint)>(",
                    ...
                    "        client: client, interface: \(P)Server.self,",
...
if mode == .client, let dispatchDir {
    ...
    for proto in closure {
        for iface in proto.interfaces {
            ...
            if iface.events.flatMap(\.args).contains(where: {
            ...
            let eventMembers = iface.events.map { event in
                let params = (["_ proxy: WaylandBorrowedProxy<\(P)Client>"]
                    + event.args.map {
                        "\(esc($0.name)): \(clientEventSwiftType($0, in: iface))"
                    }).joined(separator: ", ")
                return clientDeclaration([
                    "func \(esc(lowerCamel(event.name)))(\(params))"
                ])
            }
            let events = ProtocolDeclSyntax(
                modifiers: [DeclModifierSyntax(name: .keyword(.public))],
                name: .identifier("\(P)Events"),
                inheritanceClause: InheritanceClauseSyntax {
                    InheritedTypeSyntax(
                        type: TypeSyntax(stringLiteral: "AnyObject"))
                }
            ...
            var clientMembers: [DeclSyntax] = [
                ...
                clientDeclaration([
                    ...
                    "    unsafe \(iface.name)_add_listener(proxy, listener, Unmanaged.passUnretained(owner).toOpaque())",
                    "}",
                ]),
                clientDeclaration([
                    "private static func handler(_ data: UnsafeMutableRawPointer) -> any \(P)Events? {",
                    "    unsafe Unmanaged<AnyObject>.fromOpaque(data).takeUnretainedValue() as? any \(P)Events",
                    "}",
                ]),
            ]
            ...
            for e in iface.events {
                ...
                let args = (["WaylandBorrowedProxy<\(P)Client>(proxy)"] + call)
                    .joined(separator: ", ")
                clientMembers.append(clientDeclaration([
                    "private static let \(vname): @convention(c) (\(cparams)) -> Void = { \(cp) in",
                    "    guard let data = unsafe data, let proxy = unsafe proxy, let h = unsafe handler(data) else { return }",
        ...
        for reference in protocolDocument.references where !allDefined.contains(reference) {
            guard let dependencyPath = index[reference] else {
                throw WaylandGeneratorDiagnostic(
                    ...
                    context: reference,
                    problem: "referenced interface is not defined by the selected closure or search directories")
            }
                    ...
                    for argument in message.arguments {
                        ...
                        guard supportedArgumentTypes.contains(argument.type) else {
                            throw WaylandGeneratorDiagnostic(
                                path: protocolDocument.xmlPath,
                                context: context,
                                problem: "unsupported \(messageKind) argument type '\(argument.type)'")
                        }
                        ...
                        let components = enumReference.split(separator: ".", omittingEmptySubsequences: false)
                        ...
                        guard let targetInterface = interfaces[enumInterface]?.interface,
                            ...
                            let localExists = enumInterface == interface.name
                        ...
                        guard argument.type == "int" || argument.type == "uint" else {
                            throw WaylandGeneratorDiagnostic(
                                path: protocolDocument.xmlPath,
                                context: context,
                                problem: "enum reference is attached to non-integer type '\(argument.type)'")
                        }
                    }
                }
            }

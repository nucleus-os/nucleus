...
if mode == .server, let dispatchDir {
    ...
    for proto in closure {
        for iface in proto.interfaces {
            ...
            let exposesWaylandServer = iface.requests.contains {
                $0.args.contains { $0.type == "new_id" }
            }
            var s = """
            ...
            \(exposesWaylandServer ? "public " : "")import WaylandServer
            ...
            s += "    private static func handler(_ res: UnsafeMutablePointer<wl_resource>) -> (any \(P)Requests)? {\n"
            s += "        guard let ud = wl_resource_get_user_data(res) else { return nil }\n"
            s += "        return Unmanaged<AnyObject>.fromOpaque(ud).takeUnretainedValue() as? any \(P)Requests\n    }\n\n"
...
if mode == .client, let dispatchDir {
    ...
    for proto in closure {
        for iface in proto.interfaces {
            ...
            s += "    private static func handler(_ data: UnsafeMutableRawPointer) -> (any \(P)Events)? {\n"
            s += "        Unmanaged<AnyObject>.fromOpaque(data).takeUnretainedValue() as? any \(P)Events\n    }\n\n"

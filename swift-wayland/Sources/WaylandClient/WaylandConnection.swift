...
@safe public final class WaylandConnection {
    ...
    public var fd: Int32 { unsafe wl_display_get_fd(display) }

    /// The registry proxy (each call returns a fresh wl_registry). A WaylandRegistry owns one.
    public func getRegistry() -> OpaquePointer? { unsafe wl_display_get_registry(display) }

    /// Dispatch only events already buffered, without blocking on the socket.
    @discardableResult
    public func dispatchPending() -> Int32 { unsafe wl_display_dispatch_pending(display) }
    ...
    @discardableResult
    public func flush() -> Int32 { unsafe wl_display_flush(display) }

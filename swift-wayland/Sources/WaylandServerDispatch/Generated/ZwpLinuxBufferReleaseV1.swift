// Generated by SwiftWaylandGen. Do not edit.
// Typed server descriptor and dispatch for zwp_linux_buffer_release_v1.

package import WaylandServer
package import WaylandServerC

package enum ZwpLinuxBufferReleaseV1Server: WaylandServerInterface {
    package nonisolated static let maximumVersion: Int32 = 1
    package nonisolated static let descriptor = unsafe WaylandServerInterfaceDescriptor(
        nativeInterface: swift_wayland_iface_zwp_linux_buffer_release_v1(),
        nativeRequestVtable: nil)
    package static func sendFencedRelease(_ target: UnsafeMutablePointer<wl_resource>, fence: Int32)
    {
        unsafe zwp_linux_buffer_release_v1_send_fenced_release(target, fence)
    }
    package static func sendImmediateRelease(_ target: UnsafeMutablePointer<wl_resource>) {
        unsafe zwp_linux_buffer_release_v1_send_immediate_release(target)
    }
}
extension WaylandResourceHandle where Interface == ZwpLinuxBufferReleaseV1Server {
    @discardableResult
    package func sendFencedRelease(fence: Int32) -> Bool {
        guard let target = unsafe resource else {
            return false
        }
        unsafe zwp_linux_buffer_release_v1_send_fenced_release(target, fence)
        return true
    }
    @discardableResult
    package func sendImmediateRelease() -> Bool {
        guard let target = unsafe resource else {
            return false
        }
        unsafe zwp_linux_buffer_release_v1_send_immediate_release(target)
        return true
    }
}
extension WlNewId where Interface == ZwpLinuxBufferReleaseV1Server {
    @discardableResult
    @MainActor
    package func create<Owner: AnyObject>(
        owner: (WaylandResourceHandle<ZwpLinuxBufferReleaseV1Server>) -> Owner?,
        installed: (Owner) -> Void = { _ in
        }
    ) -> Owner? {
        unsafe _create(
            vtable: ZwpLinuxBufferReleaseV1Server.descriptor.nativeRequestVtable, owner: owner,
            installed: installed)
    }
}
extension ZwpLinuxBufferReleaseV1Server {
    @MainActor
    package static func global<Implementation: AnyObject & AnyObject>(
        implementation: Implementation,
        advertisedVersion: Int32 = maximumVersion,
        installed:
            @escaping (Implementation, WaylandResourceHandle<ZwpLinuxBufferReleaseV1Server>) -> Void =
            { _, _ in
            }
    ) -> WaylandGlobalSpecification<ZwpLinuxBufferReleaseV1Server> {
        unsafe WaylandGlobalSpecification(
            implementation: implementation,
            advertisedVersion: advertisedVersion,
            vtable: descriptor.nativeRequestVtable,
            owner: { implementation, _ in
                implementation
            },
            installed: { implementation, _, handle in
                installed(implementation, handle)
            })
    }
    @MainActor
    package static func global<Implementation: AnyObject, Owner: AnyObject>(
        implementation: Implementation,
        advertisedVersion: Int32 = maximumVersion,
        owner:
            @escaping (Implementation, WaylandResourceHandle<ZwpLinuxBufferReleaseV1Server>) ->
            Owner?,
        installed:
            @escaping (Implementation, Owner, WaylandResourceHandle<ZwpLinuxBufferReleaseV1Server>)
            -> Void = { _, _, _ in
            }
    ) -> WaylandGlobalSpecification<ZwpLinuxBufferReleaseV1Server> {
        unsafe WaylandGlobalSpecification(
            implementation: implementation,
            advertisedVersion: advertisedVersion,
            vtable: descriptor.nativeRequestVtable, owner: owner,
            installed: installed)
    }
}

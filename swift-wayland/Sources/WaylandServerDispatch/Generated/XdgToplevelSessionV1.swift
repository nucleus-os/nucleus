...
package enum XdgToplevelSessionV1Server: WaylandServerInterface {
    ...
    nonisolated(unsafe) package static let nativeRequestVtable: UnsafeRawPointer = {
        ...
        let raw = UnsafeMutableRawPointer.allocate(
            byteCount: size,
            alignment: MemoryLayout<swift_wayland_xdg_toplevel_session_v1_requests>.alignment)
        unsafe raw.initializeMemory(as: UInt8.self, repeating: 0, count: size)
        let vt = unsafe raw.bindMemory(
            to: swift_wayland_xdg_toplevel_session_v1_requests.self, capacity: 1)
            ...
            if let h = unsafe handler(res) {
                ...
                MainActor.assumeIsolated {
                    unsafe requestHandler.destroy(
                        WaylandRequest<XdgToplevelSessionV1Server>(requestResource))
                }
            } else {
                unsafe wl_resource_destroy(res)
            }
        }
    private static let rename_impl:
...
package extension WlNewId where Interface == XdgToplevelSessionV1Server {
    ...
    package func create<Owner: XdgToplevelSessionV1Requests>(
        ...
        unsafe _create(
            vtable: XdgToplevelSessionV1Server.descriptor.nativeRequestVtable, owner: owner,
            installed: installed)
    }
}
package extension XdgToplevelSessionV1Server {
    ...
    package static func global<Implementation: AnyObject & XdgToplevelSessionV1Requests>(
        ...
        installed:
            @escaping (Implementation, WaylandResourceHandle<XdgToplevelSessionV1Server>) -> Void =
            { _, _ in
            }
    ...
    @MainActor
    package static func global<Implementation: AnyObject, Owner: XdgToplevelSessionV1Requests>(
        implementation: Implementation,
        advertisedVersion: Int32 = maximumVersion,
        owner:
            @escaping (Implementation, WaylandResourceHandle<XdgToplevelSessionV1Server>) -> Owner?,
        installed:
            @escaping (Implementation, Owner, WaylandResourceHandle<XdgToplevelSessionV1Server>) ->
            Void = { _, _, _ in
            }
    ) -> WaylandGlobalSpecification<XdgToplevelSessionV1Server> {
        unsafe WaylandGlobalSpecification(

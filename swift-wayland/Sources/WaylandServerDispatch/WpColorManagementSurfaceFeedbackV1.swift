...
public protocol WpColorManagementSurfaceFeedbackV1Requests: AnyObject {
    ...
    func getPreferred(_ resource: UnsafeMutablePointer<wl_resource>, image_description: WlNewId)
    func getPreferredParametric(_ resource: UnsafeMutablePointer<wl_resource>, image_description: WlNewId)
}
...
public enum WpColorManagementSurfaceFeedbackV1Server {
    public nonisolated(unsafe) static let vtable: UnsafeRawPointer = {
        ...
        let vt = raw.bindMemory(to: swift_wayland_wp_color_management_surface_feedback_v1_requests.self, capacity: 1)
        vt.pointee.destroy = destroy_impl
        vt.pointee.get_preferred = getPreferred_impl
        vt.pointee.get_preferred_parametric = getPreferredParametric_impl
        return UnsafeRawPointer(raw)
    }()

    public static func sendPreferredChanged(_ target: UnsafeMutablePointer<wl_resource>, identity: UInt32) {
        wp_color_management_surface_feedback_v1_send_preferred_changed(target, identity)
    }

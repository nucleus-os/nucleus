// Generated by SwiftWaylandGen. Do not edit.
// Typed server descriptor and dispatch for zxdg_popup_v6.

public import WaylandServerC
public import WaylandServer
public import WaylandProtocolTypes
@MainActor public protocol ZxdgPopupV6Requests: AnyObject {
    func destroy(_ request: WaylandRequest<ZxdgPopupV6Server>)
    func grab(_ request: WaylandRequest<ZxdgPopupV6Server>, seat: WaylandBorrowedObject<WlSeatServer>, serial: UInt32)
}
public extension ZxdgPopupV6Requests {
    func destroy(_ request: WaylandRequest<ZxdgPopupV6Server>) {
        unsafe wl_resource_destroy(request.resource)
    }
}
public enum ZxdgPopupV6Server: WaylandServerInterface {
    public nonisolated(unsafe) static let interface = unsafe swift_wayland_iface_zxdg_popup_v6()
    public nonisolated static let maximumVersion: Int32 = 1
    public nonisolated(unsafe) static let vtable: UnsafeRawPointer = {
        let size = MemoryLayout<swift_wayland_zxdg_popup_v6_requests>.stride
        let raw = UnsafeMutableRawPointer.allocate(
            byteCount: size, alignment: MemoryLayout<swift_wayland_zxdg_popup_v6_requests>.alignment)
        unsafe raw.initializeMemory(as: UInt8.self, repeating: 0, count: size)
        let vt = unsafe raw.bindMemory(to: swift_wayland_zxdg_popup_v6_requests.self, capacity: 1)
        unsafe vt.pointee.destroy = destroy_impl
        unsafe vt.pointee.grab = grab_impl
        return UnsafeRawPointer(raw)
    }()
    public nonisolated static func requestVtable() -> UnsafeRawPointer? {
        unsafe vtable
    }
    public static func sendConfigure(_ target: UnsafeMutablePointer<wl_resource>, x: Int32, y: Int32, width: Int32, height: Int32) {
        unsafe zxdg_popup_v6_send_configure(target, x, y, width, height)
    }
    public static func sendPopupDone(_ target: UnsafeMutablePointer<wl_resource>) {
        unsafe zxdg_popup_v6_send_popup_done(target)
    }
    private static func handler(_ res: UnsafeMutablePointer<wl_resource>) -> any ZxdgPopupV6Requests? {
        guard let ud = unsafe wl_resource_get_user_data(res) else {
            return nil
        }
        return unsafe Unmanaged<AnyObject>.fromOpaque(ud).takeUnretainedValue() as? any ZxdgPopupV6Requests
    }
    private static let destroy_impl: @convention(c) (OpaquePointer?, UnsafeMutablePointer<wl_resource>?) -> Void = { _, res in
        guard let res = unsafe res else {
            return
        }
        if let h = unsafe handler(res) {
            nonisolated(unsafe) let requestHandler = h
            nonisolated(unsafe) let requestResource = unsafe res
            MainActor.assumeIsolated {
                unsafe requestHandler.destroy(WaylandRequest<ZxdgPopupV6Server>(requestResource))
            }
        } else {
            unsafe wl_resource_destroy(res)
        }
    }
    private static let grab_impl: @convention(c) (OpaquePointer?, UnsafeMutablePointer<wl_resource>?, UnsafeMutablePointer<wl_resource>?, UInt32) -> Void = { _, res, seat, serial in
        guard let res = unsafe res, let h = unsafe handler(res) else {
            return
        }
        nonisolated(unsafe) let requestHandler = h
        nonisolated(unsafe) let requestResource = unsafe res
        nonisolated(unsafe) let _request_seat = unsafe seat
        MainActor.assumeIsolated {
            unsafe requestHandler.grab(WaylandRequest<ZxdgPopupV6Server>(requestResource), seat: WaylandBorrowedObject<WlSeatServer>(_request_seat!), serial: serial)
        }
    }
}
public extension WaylandResourceHandle where Interface == ZxdgPopupV6Server {
    @discardableResult
    func sendConfigure(x: Int32, y: Int32, width: Int32, height: Int32) -> Bool {
        guard let target = unsafe resource else {
            return false
        }
        unsafe zxdg_popup_v6_send_configure(target, x, y, width, height)
        return true
    }
    @discardableResult
    func sendPopupDone() -> Bool {
        guard let target = unsafe resource else {
            return false
        }
        unsafe zxdg_popup_v6_send_popup_done(target)
        return true
    }
}
public extension WaylandRequest where Interface == ZxdgPopupV6Server {
    func postError(_ code: ZxdgPopupV6Error, message: String) {
        postError(code: code.rawValue, message: message)
    }
}
public extension WaylandResourceHandle where Interface == ZxdgPopupV6Server {
    @discardableResult
    func postError(_ code: ZxdgPopupV6Error, message: String) -> Bool {
        postError(code: code.rawValue, message: message)
    }
}
public extension WlNewId where Interface == ZxdgPopupV6Server {
    @discardableResult
    @MainActor
    func create<Owner: ZxdgPopupV6Requests>(
        owner: (WaylandResourceHandle<ZxdgPopupV6Server>) -> Owner?,
        installed: (Owner) -> Void = { _ in
        }
    ) -> Owner? {
        unsafe _create(vtable: ZxdgPopupV6Server.requestVtable(), owner: owner, installed: installed)
    }
}
public extension ZxdgPopupV6Server {
    @discardableResult
    @MainActor
    static func createResource<Owner: ZxdgPopupV6Requests>(
        client: OpaquePointer,
        version: Int32,
        id: UInt32 = 0,
        owner: (WaylandResourceHandle<ZxdgPopupV6Server>) -> Owner?,
        installed: (Owner) -> Void = { _ in
        }
    ) -> Owner? {
        unsafe WaylandResource.create(
            client: client, interface: ZxdgPopupV6Server.self,
            version: Swift.min(version, maximumVersion), id: id,
            vtable: requestVtable(), owner: owner,
            installed: installed)
    }
    @MainActor
    static func global<Implementation: AnyObject, Owner: ZxdgPopupV6Requests>(
        implementation: Implementation,
        advertisedVersion: Int32 = maximumVersion,
        owner: @escaping (Implementation, WaylandResourceHandle<ZxdgPopupV6Server>) -> Owner?,
        installed: @escaping (Implementation, Owner, WaylandResourceHandle<ZxdgPopupV6Server>) -> Void = { _, _, _ in
        }
    ) -> WaylandGlobalSpecification<ZxdgPopupV6Server> {
        unsafe WaylandGlobalSpecification(
            implementation: implementation,
            advertisedVersion: advertisedVersion,
            vtable: requestVtable(), owner: owner,
            installed: installed)
    }
}
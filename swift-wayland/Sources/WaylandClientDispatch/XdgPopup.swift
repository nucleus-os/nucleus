...
public protocol XdgPopupEvents: AnyObject {
    ...
    func repositioned(_ proxy: OpaquePointer, token: UInt32)
}

public enum XdgPopupClient {
    public nonisolated(unsafe) static let listener: UnsafeMutablePointer<xdg_popup_listener> = {
        let p = UnsafeMutablePointer<xdg_popup_listener>.allocate(capacity: 1)
        p.initialize(to: xdg_popup_listener())
        p.pointee.configure = configure_impl
        p.pointee.popup_done = popupDone_impl
        p.pointee.repositioned = repositioned_impl
        return p
    }()
    ...
    private static let repositioned_impl: @convention(c) (UnsafeMutableRawPointer?, OpaquePointer?, UInt32) -> Void = { data, proxy, token in
        guard let data, let proxy, let h = handler(data) else { return }
        h.repositioned(proxy, token: token)
    }
}

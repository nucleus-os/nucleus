...
@MainActor
private final class RecordingAtSPIExport {
    ...
    func bridge(scene: WindowScene) -> AtSPIBridge {
        AtSPIBridge(scene: scene) { [self] snapshot, update in
...
struct AtSPIExportModelTests {
    ...
    @Test func stablePathsRolesStatesAndActionsSurviveUpdates() throws {
        ...
        let adapter = RecordingAtSPIExport(applicationName: "Settings")
        let bridge = adapter.bridge(scene: scene)
        var presses = 0
    ...
    @Test func secureTextNeverAdvertisesTextOrEditableText() throws {
        ...
        let adapter = RecordingAtSPIExport()
        let bridge = adapter.bridge(scene: scene)
    ...
    @Test func propertyStateAndTextDiffsProduceEventsWithoutSecureText()
        throws
    {
        let root = View()
        let button = Button(title: "Before")
        let field = TextField(string: "before")
        let secure = TextField(string: "secret-before", isSecure: true)
        root.addSubview(button)
        ...
        secure.stringValue = "secret-after"
        _ = bridge.publish()

        let events = try #require(adapter.updates.last?.events)
        #expect(events.contains {
            $0.kind == .propertyChanged
                && $0.sourcePath == AtSPIExportModel.path(
                    for: button.accessibilityID)
                && $0.text == "After"
        })
        #expect(events.contains {

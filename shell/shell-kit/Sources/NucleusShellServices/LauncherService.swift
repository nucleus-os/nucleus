internal import Foundation  // Process and FileHandle implement detached application launching.
...
final class DesktopProcessLauncher {
    ...
    private static func containsFirefoxProfileOverride(_ args: [String]) -> Bool {
        args.contains { arg in
            arg.hasPrefix("-P") || arg == "--profile" || arg.hasPrefix("--profile=")
                || arg == "-profile"
    ...
    private static func expandedEnvSplitArguments(_ args: [String]) -> [String] {
        ...
        while index < args.count {
            ...
            if arg.hasPrefix("--split-string=") {
                expanded.append(
                    contentsOf: splitEnvString(String(arg.dropFirst("--split-string=".count))))
                index += 1
                continue
            }
    ...
    private static func sanitizedSessionEnvironmentOverrides(_ args: [String]) -> [String] {
        ...
        while index < args.count {
            ...
            if arg.hasPrefix("--chdir=") || arg.hasPrefix("--argv0=")
                || arg.hasPrefix("--split-string=")
    ...
    private nonisolated static func loginShell() -> String? {
        ...
        let bufferSize =
            configuredSize > 0 && configuredSize <= 1 << 20
            ? configuredSize
            : 16 * 1024

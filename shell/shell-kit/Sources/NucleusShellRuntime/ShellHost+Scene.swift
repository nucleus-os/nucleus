...
extension ShellHost {
    ...
    func setupInput() {
        ...
        let seat = NucleusDesktopSeat(client: client)
        if seat == nil {
        ...
        let router = NucleusDesktopInputRouter(scene: scene, seat: seat, client: client)
    ...
    func waylandGlobalChanged(_ kind: NucleusDesktopCapabilityKind) {
        switch kind {
        case .dataControl:
            configurePasteboard(for: seat)
        case .dataDevice:
        ...
        case .seat:
            ...
            dragDropAdapter = nil
            let replacement = NucleusDesktopSeat(client: client)
            seat = replacement
    ...
    func configurePasteboard(for seat: NucleusDesktopSeat?) {
    ...
    func configureDragDrop(for seat: NucleusDesktopSeat?) {
        dragDropAdapter?.shutdown()
        dragDropAdapter = nil
        guard let seat, let router = inputRouter else { return }
        dragDropAdapter = NucleusDesktopDragDropAdapter(
            client: client,
            seat: seat,
    ...
    static func cursorShape(for cursor: Cursor) -> NucleusDesktopCursorShape {
        switch cursor {

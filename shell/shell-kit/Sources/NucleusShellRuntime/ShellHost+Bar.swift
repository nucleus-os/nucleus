...
extension ShellHost {
    ...
    func setupProduct() {
        ...
        actionDispatcher.onFeedbackChanged = { [weak self] state in
            self?.reconcileFeedbackSurface(state)
        }
        notifications.onChanged = { [weak self] notifications in
            self?.reconcileNotificationSurface(notifications)
        }
    ...
    func outputsChanged() {
        ...
        if actionDispatcher.feedback != .hidden {
            reconcileFeedbackSurface(actionDispatcher.feedback)
        }
        if !notifications.notifications.isEmpty {
            reconcileNotificationSurface(notifications.notifications)
        }
        updateSceneDisplayBounds()
    ...
    func setupForeignToplevel() {
        guard let manager = NucleusDesktopForeignToplevelManager(client: client) else {

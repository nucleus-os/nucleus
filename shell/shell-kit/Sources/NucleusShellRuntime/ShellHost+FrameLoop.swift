...
extension ShellHost {
    ...
    func flushDisplay(needsWrite: inout Bool) -> Bool {
        ...
        let disposition = NucleusWindowClientFlushDisposition.classify(
            result: result,
            error: errno)
        switch disposition {
        case .flushed:
    ...
    func renderFrameIfDue(counters: inout ShellLoopCounters) {
        ...
        if engine.deviceLost {
            writeErr(
                "nucleus-shell: renderer device lost; terminating client")
            running = false
            return
        }
        ...
        nextPresentationDeadlineNs = NucleusWindowClientPresentationTiming.nextDeadline(
    ...
    private func noteStartupPresentations(
        ...
        let wallpapers = wallpaperSurfaces.values.map { record in
            NucleusWindowClientStartupSurface(

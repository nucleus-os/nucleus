...
source "$script_dir/scripts/patch-stack.sh"
...
profiles, Dawn checkout, and compiler cache. CEF and Nucleus Browser use
...
build_browser() {
  ...
  nucleus_apply_patch_stack \
    ...
    "$script_dir/patches/browser" \
    "$NUCLEUS_CEF_SRC_ROOT/.nucleus-applied-browser-patches" \
    "Nucleus Browser"

  if [[ $prepare_only -eq 1 ]]; then
    echo
    echo "== Nucleus Browser source prepared =="
  ...
  echo "-- generating Nucleus Browser output: $NUCLEUS_BROWSER_OUT"
  (
    cd "$NUCLEUS_CHROMIUM_SRC_ROOT"
    "$NUCLEUS_CHROMIUM_SRC_ROOT/buildtools/linux64/gn" gen \
      "$NUCLEUS_BROWSER_OUT" \
      --args="$NUCLEUS_BROWSER_GN_DEFINES_BASE $NUCLEUS_BROWSER_GN_EXTRA"
  )

  echo "-- building Nucleus Browser"
  autoninja -j "$NUCLEUS_CEF_JOBS" -C "$NUCLEUS_BROWSER_OUT" \
    chrome chrome_sandbox

  echo
  echo "== Nucleus Browser build complete =="
  echo "binary: $NUCLEUS_BROWSER_OUT/chrome"
  echo "The product remains a development artifact until the Wayland presenter"
  echo "and packaging acceptance phases in docs/nucleus-browser-plan.md pass."
}
...
case "$product" in
  ...
  all)
    build_cef
    # CEF has already synchronized and prepared the shared checkout. Reuse it
    # for the browser layer even for --prepare-only; in particular, never run
    # a requested --force-clean twice.
    cef_args=(--no-update --skip-deps)
    build_browser
    ;;
esac

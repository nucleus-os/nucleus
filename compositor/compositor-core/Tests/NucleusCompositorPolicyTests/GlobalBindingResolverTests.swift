...
@Suite @MainActor struct GlobalBindingResolverTests {
    // Held for the test's lifetime: GlobalBindingResolver keeps the window manager
    ...
    private func service(_ binds: [KeyBind]) -> GlobalBindingResolver {
        GlobalBindingResolver(
            windowManager: windowManager,
            binds: binds)
    }

    private func isDeferred(_ dispatch: GlobalBindingResolver.Dispatch) -> Bool {
        if case .deferred = dispatch { return true }
        return false
    }

    private func isPass(_ dispatch: GlobalBindingResolver.Dispatch) -> Bool {
    ...
    @Test func tileDirectionsMapOntoTheTileCommandWireValues() {
        #expect(GlobalBindingResolver.wireValue(.left) == 1)
        #expect(GlobalBindingResolver.wireValue(.right) == 2)
        #expect(GlobalBindingResolver.wireValue(.top) == 3)
        #expect(GlobalBindingResolver.wireValue(.bottom) == 4)
        #expect(GlobalBindingResolver.wireValue(.topLeft) == 5)
        #expect(GlobalBindingResolver.wireValue(.topRight) == 6)
        #expect(GlobalBindingResolver.wireValue(.bottomLeft) == 7)
        #expect(GlobalBindingResolver.wireValue(.bottomRight) == 8)
        #expect(GlobalBindingResolver.wireValue(.maximize) == 9)
    }

    @Test func reactorModifierBitsDecodeToTheSharedOptionSet() {
        // CGEventFlags-shaped packing: shift=17, control=18, alt=19, super=20.
        #expect(GlobalBindingResolver.decode(modifierBits: 1 << 17) == .shift)
        #expect(GlobalBindingResolver.decode(modifierBits: 1 << 18) == .control)
        #expect(GlobalBindingResolver.decode(modifierBits: 1 << 19) == .alt)
        #expect(GlobalBindingResolver.decode(modifierBits: 1 << 20) == .superKey)
        #expect(GlobalBindingResolver.decode(modifierBits: (1 << 17) | (1 << 20))
            == [.shift, .superKey])
        #expect(GlobalBindingResolver.decode(modifierBits: 0) == [])
    }

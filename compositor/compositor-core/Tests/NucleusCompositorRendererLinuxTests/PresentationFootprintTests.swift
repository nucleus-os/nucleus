...
@Suite struct PresentationFootprintTests {
    ...
    @Test func physicalDamageRoundsOutward() {
        ...
        #expect(
            physicalDamageRectFromLogicalRect(
                Self.target(), LogicalRect(x: 0, y: 0, width: 0, height: 5)) == nil,
    ...
    @Test func layerFootprintShadowExtent() {
        ...
        style.shadow = LayerShadow(
            offsetX: 0, offsetY: 12, blurRadius: 28, spreadRadius: 0,
            cornerRadius: 18, color: .init(0, 0, 0, 0.28))
        ...
        #expect(
            Self.approxD(footprint.visualOutset.x, 42)
                && Self.approxD(footprint.visualOutset.y, 54), "shadow-outset")
        #expect(
            footprint.visualRect == LogicalRect(x: 58, y: -4, width: 284, height: 188),
            "shadow-visual-rect")
    ...
    @Test func decorationSlotShadowMargin() {
        ...
        let footprint = computeLayerFootprint(
            LayerFootprintInput(
                ...
                decorationSlot: slot))
        #expect(
            Self.approxD(footprint.visualOutset.x, 30)
                && Self.approxD(footprint.visualOutset.y, 40), "deco-slot-outset")
    }

    @Test func clipTrimsContentAndVisual() {
        let layer = Layer(id: 8, kind: .container)
        let clip = ClipState.rect(
            RoundedClip(rect: LogicalRect(x: 0, y: 0, width: 150, height: 150)))
        let footprint = computeLayerFootprint(
            LayerFootprintInput(
                layer: layer, bounds: Bounds(w: 200, h: 200),
                layerRect: LogicalRect(x: 100, y: 100, width: 200, height: 200), clip: clip))
        #expect(
            footprint.visibleContentRect == LogicalRect(x: 100, y: 100, width: 50, height: 50),
            "clip-content")
        #expect(footprint.physicalDamageRect(Self.target()) != nil, "clip-has-damage")
    }

    @Test func planRectProjection() {
        let pr = planRectFromLogicalRect(
            Self.target(), LogicalRect(x: 10, y: 20, width: 30, height: 40))
    ...
    @Test func stableModelRectMapping() {
        ...
        #expect(
            Self.approxD(r.x, 10) && Self.approxD(r.y, 20) && Self.approxD(r.width, 100)
                && Self.approxD(r.height, 50),
            "stable-model-rect")
    }
}

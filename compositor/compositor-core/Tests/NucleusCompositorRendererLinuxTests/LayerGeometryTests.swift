...
@Suite struct LayerGeometryTests {
    ...
    @Test func m44Mapping() {
        ...
        #expect(
            Self.approx(r.x, 5) && Self.approx(r.y, 6) && Self.approx(r.w, 10)
                && Self.approx(r.h, 20), "m44-maprect")
        // from3x3 identity behaves as identity.
        let id3 = M44.from3x3([1, 0, 0, 0, 1, 0, 0, 0, 1])
        #expect(
            Self.approx(id3.mapPoint(7, 8).x, 7) && Self.approx(id3.mapPoint(7, 8).y, 8),
            "m44-from3x3")
    }
    ...
    @Test func localCompositionMatrix() {
        ...
        #expect(
            Self.approx(m.mapPoint(0, 0).x, 10) && Self.approx(m.mapPoint(0, 0).y, 20),
            "compose-anchor-cancels")
        ...
        #expect(
            Self.approx(s.mapPoint(50, 25).x, 50) && Self.approx(s.mapPoint(50, 25).y, 25),
            "compose-scale-center-fixed")
    }

    @Test func scaledClipForBounds() {
        // doubles rect + radii on a 2× bounds growth.
        let clip = RenderClip(
            rect: (0, 0, 100, 50), radii: (8, 8, 8, 8), antiAlias: true,
    ...
    @Test func layerClipRectMapping() {
        ...
        layer.model.properties.clip = RenderClip(
            rect: (10, 10, 30, 20), radii: (4, 4, 4, 4),
            antiAlias: true, transform: [1, 0, 0, 0, 1, 0, 0, 0, 1])
        let rc = layerClipRect(layer, M44.identity)!
        #expect(
            Self.approxD(rc.rect.x, 10) && Self.approxD(rc.rect.y, 10)
                && Self.approxD(rc.rect.width, 30) && Self.approxD(rc.rect.height, 20),
            "layer-clip-rect")
    ...
    @Test func accumulateClipDownTree() {
        ...
        let parent = ClipState.rect(
            RoundedClip(rect: LogicalRect(x: 0, y: 0, width: 100, height: 100)))
        var layer = Layer(id: 2, kind: .container)
        layer.model.properties.bounds = Bounds(w: 100, h: 100)
        layer.model.properties.clip = RenderClip(
            rect: (50, 50, 100, 100), radii: (0, 0, 0, 0),
            antiAlias: false, transform: [1, 0, 0, 0, 1, 0, 0, 0, 1])
        let acc = accumulateClip(parent, layer, M44.identity)
        let r = clipRect(acc)!
        #expect(
            Self.approxD(r.x, 50) && Self.approxD(r.y, 50) && Self.approxD(r.width, 50)
                && Self.approxD(r.height, 50), "accumulate-clip")
        let trimmed = clipLayerRect(acc, LogicalRect(x: 0, y: 0, width: 200, height: 200))!
        #expect(
            Self.approxD(trimmed.width, 50) && Self.approxD(trimmed.height, 50), "clip-layer-rect")
        ...
        disjoint.model.properties.clip = RenderClip(
            rect: (500, 500, 50, 50), radii: (0, 0, 0, 0),
            antiAlias: false, transform: [1, 0, 0, 0, 1, 0, 0, 0, 1])
        if case .empty = accumulateClip(parent, disjoint, M44.identity) {
            #expect(true, "accumulate-empty")
        } else {
            #expect(Bool(false), "accumulate-empty")
        }
    }
    ...
    @Test func intersectionHelpers() {
        ...
        #expect(
            intersectLogicalRects(a, b) == LogicalRect(x: 50, y: 50, width: 50, height: 50),
            "intersect")
        #expect(Self.approxD(rectIntersectionArea(a, b), 2500), "intersect-area")
        #expect(
            intersectLogicalRects(a, LogicalRect(x: 200, y: 200, width: 10, height: 10)) == nil,
            "intersect-disjoint")
    }

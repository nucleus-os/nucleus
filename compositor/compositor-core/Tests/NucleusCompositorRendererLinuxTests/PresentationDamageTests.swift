...
@Suite struct PresentationDamageTests {
    ...
    static func nativeFact(_ r: DamageRect, sig: UInt64, layer: UInt64 = 1) -> NativeLayerDamageFact
    {
        NativeLayerDamageFact(
            outputId: 1, layerId: layer, visibleRect: r,
            visualSignature: sig)
    }
    ...
    @Test func crossFrameCache() {
        ...
        state.beginFrame()
        state.nativeLayerPending[key] = NativeLayerSnapshot(
            visibleRect: Self.rect(0, 0, 10, 10), visualSignature: 1)
        state.commitFrame()
    ...
    @Test func remoteHostChangeDetection() {
        ...
        func remoteFact(_ r: DamageRect, sig: UInt64) -> RemoteHostDamageFact {
            RemoteHostDamageFact(
                outputId: 1, hostLayerId: 3, targetContextId: ContextID(raw: 2),
                rootLayerId: 10, contextRevision: 1,
                sourceRect: RenderRect(x: 0, y: 0, w: 100, h: 100),
    ...
    @Test func staleRetirement() {
        ...
        state.nativeLayers[key] = NativeLayerSnapshot(
            visibleRect: Self.rect(1, 2, 3, 4), visualSignature: 1)
    ...
    @Test func rectRegionMath() {
        ...
        #expect(
            intersectDamageRects(Self.rect(0, 0, 10, 10), Self.rect(5, 5, 10, 10))
                == Self.rect(5, 5, 5, 5), "intersect")
        #expect(
            intersectDamageRects(Self.rect(0, 0, 4, 4), Self.rect(10, 10, 4, 4)) == nil,
            "intersect-disjoint")
        #expect(
            clampDamageRectToTarget(Self.rect(-5, -5, 20, 20), 10, 10) == Self.rect(0, 0, 10, 10),
            "clamp")
        ...
        #expect(
            abs(damageBoundsFraction(Self.rect(0, 0, 50, 100), 100, 100) - 0.5) < 1e-9, "fraction")
        #expect(
            planRectFromDamageRect(Self.rect(3, 4, 5, 6)) == PlanRect(x: 3, y: 4, w: 5, h: 6),
            "plan-rect")
        #expect(
            planRectsIntersect(
                PlanRect(x: 0, y: 0, w: 10, h: 10), PlanRect(x: 5, y: 5, w: 10, h: 10)),
            "plan-intersect")
        #expect(
            !planRectsIntersect(PlanRect(x: 0, y: 0, w: 4, h: 4), PlanRect(x: 5, y: 5, w: 4, h: 4)),
            "plan-disjoint")
    }

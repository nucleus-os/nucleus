...
@Suite struct PresentationSceneTests {
    static func approxD(_ a: Double, _ b: Double, _ eps: Double = 1e-4) -> Bool {
        abs(a - b) <= eps
    }
    ...
    @Test func lowerLayerInputGeometry() {
        ...
        let input = lowerLayerInput(
            layerId: 1, layer: layer, parentMatrix: M44.identity,
            parentOpacity: 0.8, parentClip: .none,
            layerOpacity: layer.effectiveOpacity())!
        #expect(
            Self.approxD(input.layerRect.x, 10) && Self.approxD(input.layerRect.y, 20),
            "input-rect-origin")
        #expect(
            Self.approxD(input.layerRect.width, 100) && Self.approxD(input.layerRect.height, 50),
            "input-rect-size")
    ...
    @Test func remoteHostInlineResolution() {
        ...
        let input = lowerLayerInput(
            layerId: 5, layer: layer, parentMatrix: M44.identity,
            parentOpacity: 1, parentClip: .none, layerOpacity: 1)!
        let geom = HostedContextGeometry(
            rootLayerId: 77, sourceRect: RenderRect(x: 0, y: 0, w: 100, h: 100))
        guard
            case .inlineSubtree(let inlineHost) = lowerRemoteHost(
                input, geom, destinationPassRequired: true)!
        else {
            #expect(Bool(false), "remote-inline")
            return
        }
        #expect(inlineHost.rootLayerId == 77 && inlineHost.destinationPassRequired, "remote-inline")
        if case .rect(let rc) = inlineHost.clip {
            #expect(
                Self.approxD(rc.rect.width, 100) && Self.approxD(rc.rect.height, 100), "remote-clip"
            )
        } else {
            #expect(Bool(false), "remote-clip")
        }

        let fact = lowerRemoteHostDamageFact(
            Self.target(), input, hostContextId: ContextID(raw: 9),
            geom, contextRevision: 3)!
        #expect(
            fact.hostLayerId == 5 && fact.rootLayerId == 77 && fact.contextRevision == 3,
            "remote-fact")
        #expect(
            fact.visibleRect == PhysicalRect(x: 0, y: 0, width: 200, height: 200),
            "remote-fact-rect")
    }
    ...
    @Test func damageFactLowering() {
        ...
        let rhInput = lowerLayerInput(
            layerId: 5, layer: rh, parentMatrix: M44.identity,
            parentOpacity: 1, parentClip: .none, layerOpacity: 1)!
        let hosted = RemoteHostDamageInput(
            hostContextId: ContextID(raw: 9),
            geometry: HostedContextGeometry(
                rootLayerId: 77, sourceRect: RenderRect(x: 0, y: 0, w: 100, h: 100)),
            contextRevision: 1)
        ...
        let dockInput = lowerLayerInput(
            layerId: 1, layer: dock, parentMatrix: M44.identity,
            parentOpacity: 1, parentClip: .none, layerOpacity: 1)!
        ...
        let gInput = lowerLayerInput(
            layerId: 2, layer: generic, parentMatrix: M44.identity,
            parentOpacity: 1, parentClip: .none, layerOpacity: 1)!
    ...
    @Test func externalDamageFact() {
        ...
        let input = lowerLayerInput(
            layerId: 3, layer: layer, parentMatrix: M44.identity,
            parentOpacity: 1, parentClip: .none, layerOpacity: 1)!
        let fact = lowerExternalDamageFact(Self.target(), input)!
        if case .compositorExternal = fact.source {
            #expect(true, "external-source")
        } else {
            #expect(Bool(false), "external-source")
        }
    ...
    @Test func nativeTrackingGateByRole() {
        ...
        var notif = Layer(id: 1, kind: .container)
        notif.role = .notification
    ...
    @Test func visualStateSignatures() {
        ...
        #expect(
            r1 == remoteHostSignature(ContextID(raw: 9), M44.identity, 1.0), "remote-sig-stable")
        #expect(
            r1 != remoteHostSignature(ContextID(raw: 9), M44.identity, 0.5), "remote-sig-opacity")
    }

    @Test func outputScaleGenerationAndBackdropIds() {
        let g1 = outputScaleGeneration(Self.target())
        var t2 = Self.target()
        t2.pixelSize = PixelSize(width: 800, height: 400)

...
struct WaylandProtocolConformanceTests {
    ...
    private func bind(
        ...
        guard let g = globals.first(where: { $0.interface == iface }) else {
            Issue.record("global \(iface) not advertised")
    ...
    @Test func layerShellRejectsInvalidAnchor() throws {
        ...
        WlOutput(
            info: OutputInfo(
                ...
                refreshMhz: 60000, scale: 1, name: "DP-1", description: "Out")
        ).register(in: router)
    ...
    private func stateSet(_ m: WireMessage) -> [UInt32] {
        ...
        var out: [UInt32] = []
        var off = 4
        while off + 4 <= 4 + byteLen {
            out.append(m.u32(off))
            off += 4
        }
        return out
    }
    ...
    @MainActor @Test func foreignToplevelReportsMinimizedState() throws {
        ...
        let toplevel = try #require(
            WireMessage.first(enumerated, object: mgrId, opcode: 0), "no toplevel handle")
        ...
        #expect(
            stateSet(firstState).contains(1),
            "minimized must be reported when the window is minimized")
        ...
        let restoredState = try #require(
            afterRestore.last { $0.objectId == handleId && $0.opcode == 4 },
            "no state event after restore")
        #expect(
            !stateSet(restoredState).contains(1), "minimized must clear when the window is restored"
        )
    }
    ...
    @Test func screencopyLogicalRegionsClipBeforePixelProjection() throws {
        let clipped = try #require(
            RouterRenderDriver.projectCaptureAxis(
                origin: -10,
                length: 30,
                logicalExtent: 100,
                pixelExtent: 200))
    ...
    @Test func screencopyRejectsMismatchedBuffer() throws {
        ...
        var shm = WireBuilder()
        let fd = try appendShmBuffer(
            &shm, shmId: shmId, poolId: poolId, bufId: bufId,
            width: 4, height: 4, stride: 16, format: 0)
        try client.send(shm, fd: fd)
        client.pump()
        ...
        #expect(
            WireMessage.first(client.drainEvents(), object: frameId, opcode: 0) != nil,
            "no buffer event")
    ...
    private func appendShmBuffer(
        ...
        b.message(object: shmId, opcode: 0) {
            $0.newId(poolId)
            $0.int(size)
        }  // create_pool (fd ancillary)
    ...
    @Test func syncobjPointsLatchDestroyAndRejectConflicts() throws {
        ...
        let surface = try #require(
            surface(
                in: compositor, withWireObjectID: surfaceID))
        #expect(
            surface.aux.syncAcquire
                == SyncPoint(
                    handle: syncobjDelegate.importedHandle, point: 5))
        #expect(
            surface.aux.syncRelease
                == SyncPoint(
    ...
    @Test func immediatelyReusableBufferIsReleasedExactlyOnce() throws {
        ...
        let surfaceID: UInt32 = 5
        let poolA: UInt32 = 6
        let bufferA: UInt32 = 7
        let poolB: UInt32 = 8
        let bufferB: UInt32 = 9
    ...
    @Test func importedDmabufReleaseWaitsForRendererRetirement() throws {
        ...
        let dmaID: UInt32 = 4
        let surfaceID: UInt32 = 5
        let paramsA: UInt32 = 6
        let bufferA: UInt32 = 7
        let paramsB: UInt32 = 8
        let bufferB: UInt32 = 9
        var setup = WireBuilder()
    ...
    @Test func layerShellRejectsBufferedSurface() throws {
        ...
        try bind(&a, "zwlr_layer_shell_v1", lsId, globals)
        let fd = try appendShmBuffer(
            &a, shmId: shmId, poolId: poolId, bufId: bufId,
            width: 4, height: 4, stride: 16, format: 0)
        a.message(object: compId, opcode: 0) { $0.newId(surfId) }  // create_surface
        a.message(object: surfId, opcode: 1) {
            $0.object(bufId)
            $0.int(0)
            $0.int(0)
        }  // attach
        ...
        client.pump()
        let err = try #require(
            WireError.first(client.drainEvents()), "expected already_constructed")
    ...
    @MainActor @Test func foreignToplevelUnminimizeRestoresWindow() throws {
        ...
        #expect(
            !window.minimized, "unset_minimized must restore the window (was a permanent dead-end)")
    }
    ...
    @Test func sessionLockSecondLockerIsInert() throws {
        ...
        WlOutput(
            info: OutputInfo(
                physicalWidthMm: 600, physicalHeightMm: 340, pixelWidth: 64, pixelHeight: 48,
                refreshMhz: 60000, scale: 1, name: "LOCK-1", description: "Lock")
        ).register(in: router)
        ...
        a.message(object: lock1, opcode: 1) {
            $0.newId(ls1)
            $0.object(surf1)
            $0.object(outId)
        }
        a.message(object: lock2, opcode: 1) {
            $0.newId(ls2)
            $0.object(surf2)
            $0.object(outId)
        }
        ...
        // lock2 was denied.
        #expect(
            WireMessage.first(events, object: lock2, opcode: 1) != nil, "lock2 should be finished")
        // The granted lock's surface is configured (ext_session_lock_surface_v1.configure = op 0).
        #expect(
            WireMessage.first(events, object: ls1, opcode: 0) != nil,
            "granted lock surface must configure")
        // The inert lock's get_lock_surface is ignored — no configure on ls2.
        #expect(
            WireMessage.first(events, object: ls2, opcode: 0) == nil,
            "inert lock must not configure a surface")
    }
    ...
    @Test func sessionLockRejectsMismatchedBuffer() throws {
        ...
        let cfg = try #require(
            WireMessage.first(configured, object: lsurf, opcode: 0), "no configure")
        ...
        b.message(object: surfId, opcode: 1) {
            $0.object(bufId)
            $0.int(0)
            $0.int(0)
        }  // attach 4×4
        ...
        client.pump()
        let err = try #require(
            WireError.first(client.drainEvents()), "expected dimensions_mismatch")
    ...
    @MainActor @Test func presentTickDeliversFrameCallbackAndFeedback() throws {
        ...
        a.message(object: presId, opcode: 1) {
            $0.object(surfId)
            $0.newId(fbId)

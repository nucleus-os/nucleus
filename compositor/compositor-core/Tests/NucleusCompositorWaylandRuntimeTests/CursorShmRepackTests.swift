...
@Suite struct CursorShmRepackTests {
    ...
    @Test func stripsStridePadding() {
        ...
        let out = src.withUnsafeBytes {
            unsafe PointerCursorSurface.repackTightARGB(
                source: $0, width: w, height: h, sourceStride: stride)
        }
    ...
    @Test func rejectsUndersizedStrideWithoutOverread() {
        ...
        let out = src.withUnsafeBytes {
            unsafe PointerCursorSurface.repackTightARGB(
                source: $0, width: 4, height: 1, sourceStride: 4)  // needs 16 > 4
        }
        #expect(out.count == 16)
    ...
    @Test func zeroDimensionsYieldEmptyNoCrash() {
        ...
        let out = empty.withUnsafeBytes {
            unsafe PointerCursorSurface.repackTightARGB(
                source: $0, width: 0, height: 0, sourceStride: 0)
        }
        #expect(out.isEmpty)
    }
}

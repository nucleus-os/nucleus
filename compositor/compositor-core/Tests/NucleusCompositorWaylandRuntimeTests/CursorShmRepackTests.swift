...
@Suite struct CursorShmRepackTests {
    ...
    @Test func stripsStridePadding() {
        ...
        let out = PointerCursorSurface.repackTightARGB(
            source: src, width: w, height: h, sourceStride: stride)
    ...
    @Test func rejectsUndersizedStrideWithoutOverread() {
        ...
        let out = PointerCursorSurface.repackTightARGB(
            source: src, width: 4, height: 1, sourceStride: 4)  // needs 16 > 4
        #expect(out.count == 16)
    ...
    @Test func zeroDimensionsYieldEmptyNoCrash() {
        let empty = [UInt8]()
        let out = PointerCursorSurface.repackTightARGB(
            source: empty, width: 0, height: 0, sourceStride: 0)
        #expect(out.isEmpty)
    }
}

...
package enum WindowInteraction {
    case idle
    case movePending(
        windowID: UInt64, startCursorX: Double, startCursorY: Double, startRect: WindowRect)
    case moveActive(
        windowID: UInt64, startCursorX: Double, startCursorY: Double, startRect: WindowRect)
    case resizePending(
        windowID: UInt64, startCursorX: Double, startCursorY: Double, startRect: WindowRect,
    ...
    package mutating func activate() {
        ...
        case .movePending(let windowID, let startCursorX, let startCursorY, let startRect):
            self = .moveActive(
                windowID: windowID, startCursorX: startCursorX, startCursorY: startCursorY,
                startRect: startRect)
        case .resizePending(
            let windowID, let startCursorX, let startCursorY, let startRect, let edges):
            self = .resizeActive(
                windowID: windowID, startCursorX: startCursorX, startCursorY: startCursorY,
                startRect: startRect, edges: edges)
        case .idle, .moveActive, .resizeActive:
            break
        }
    }

    package mutating func clear(forWindow id: UInt64) {
        if windowID == id {
            self = .idle
        }
    }

    package func update(cursorX: Double, cursorY: Double) -> WireInteractionGrabUpdate {
        switch self {
        case .idle:
...
@MainActor
package struct InteractionState {
    package private(set) var windowInteraction: WindowInteraction = .idle
    private var nextLayoutTransitionID: UInt64 = 1

    package init() {}

    package mutating func allocLayoutTransitionID() -> UInt64 {
    ...
    package mutating func beginInteractiveMove(
        windowID: UInt64,
        cursorX: Double,
        cursorY: Double,
        startRect: WindowRect
    ) {
        windowInteraction = .movePending(
            windowID: windowID, startCursorX: cursorX, startCursorY: cursorY, startRect: startRect)
        windowInteraction.activate()
    }

    package mutating func beginInteractiveResize(
        windowID: UInt64,
        cursorX: Double,
        cursorY: Double,
        startRect: WindowRect,
    ...
    package mutating func updateInteractiveGrab(cursorX: Double, cursorY: Double)
        -> WireInteractionGrabUpdate
    {
        windowInteraction.update(cursorX: cursorX, cursorY: cursorY)
    }

    package mutating func finishInteractiveGrab() {
        windowInteraction = .idle
    }

    package mutating func endInteractiveGrab() {
        windowInteraction = .idle
    }

    package mutating func cancelInteractiveGrab() {
        windowInteraction = .idle
    }

    package mutating func clearGrab(forWindow id: UInt64) {

...
package struct WindowStyleMask: OptionSet, Sendable, Equatable {
    ...
    package static let titled = WindowStyleMask(rawValue: 1 << 0)
    /// The titlebar shows a close button.
    package static let closable = WindowStyleMask(rawValue: 1 << 1)
    /// The titlebar shows a minimize button.
    package static let minimizable = WindowStyleMask(rawValue: 1 << 2)
    /// The titlebar shows a maximize button.
    package static let maximizable = WindowStyleMask(rawValue: 1 << 3)
    /// The window can be resized by dragging its edges.
    package static let resizable = WindowStyleMask(rawValue: 1 << 4)
    ...
    package static let fullSizeContentView = WindowStyleMask(rawValue: 1 << 5)

    /// No server chrome — the client-side-decorated / undecorated case.
    package static let borderless: WindowStyleMask = []

    /// The standard managed-app toplevel: a titlebar carrying all three controls,
    /// plus edge resizing.
    package static let titledResizable: WindowStyleMask =
...
package typealias WindowEdgeInsets = WireChromeInsets

extension WireChromeInsets {
    package static let zero = WireChromeInsets(top: 0, left: 0, bottom: 0, right: 0)

    package var horizontal: Double { left + right }
    package var vertical: Double { top + bottom }
    package var isZero: Bool { top == 0 && left == 0 && bottom == 0 && right == 0 }
}
...
package struct WindowThemeMetrics: Sendable, Equatable {
    package var titlebarHeight: Double
    package var borderThickness: Double

    package init(titlebarHeight: Double, borderThickness: Double) {
        self.titlebarHeight = titlebarHeight
        self.borderThickness = borderThickness
    }

    package static let standard = WindowThemeMetrics(titlebarHeight: 28, borderThickness: 1)
}
...
package enum WindowButton: Sendable, Equatable, CaseIterable {
    case close
    case minimize
    case maximize
}
...
package struct ResizeEdges: OptionSet, Sendable, Equatable {
    package let rawValue: UInt32
    package init(rawValue: UInt32) { self.rawValue = rawValue }

    package static let left = ResizeEdges(rawValue: 1 << 0)
    package static let right = ResizeEdges(rawValue: 1 << 1)
    package static let top = ResizeEdges(rawValue: 1 << 2)
    package static let bottom = ResizeEdges(rawValue: 1 << 3)
}
...
package enum ChromeRegion: UInt32, Sendable, Equatable {
    case content = 0
    case titlebar = 1
    case resize = 2
    case closeButton = 3
...
package struct ChromeHit: Sendable, Equatable {
    package var region: ChromeRegion
    package var edges: ResizeEdges

    package init(region: ChromeRegion, edges: ResizeEdges = []) {
        self.region = region
        self.edges = edges
    }

    package static let content = ChromeHit(region: .content)

    /// Region in the low byte, edges in the next byte — the wire form the
    /// compositor decodes during hit-testing.
    package var packed: UInt64 { UInt64(region.rawValue) | (UInt64(edges.rawValue) << 8) }
}
...
package struct FrameLocalRect: Sendable, Equatable {
...
package struct WindowFrameView: Sendable, Equatable {
    package var styleMask: WindowStyleMask
    package var metrics: WindowThemeMetrics
    ...
    package var hasTitlebar: Bool { !fullscreen && styleMask.contains(.titled) }

    /// Height of the titlebar band, or zero when no titlebar is shown.
    package var titlebarHeight: Double { hasTitlebar ? metrics.titlebarHeight : 0 }
    ...
    package var contentInsets: WindowEdgeInsets {
    ...
    package var windowMenuCapabilities: UInt32 {
        var caps: UInt32 = 0
    ...
    package var standardButtons: [WindowButton] {
    ...
    package enum ButtonLayout {
        package static let diameter: Double = 12
        /// Center-to-center spacing between adjacent controls.
        package static let spacing: Double = 20
        /// Cluster start, measured from the frame's leading edge to the first
        /// control's center.
        package static let leadingInset: Double = 20
    }
    ...
    package func rect(for button: WindowButton) -> FrameLocalRect? {
    ...
    package func button(atFrameLocal point: FrameLocalRect) -> WindowButton? {
    ...
    package enum ResizeGrab {
        package static let border: Double = 6
        package static let outset: Double = 4
    }
    ...
    package func classify(x: Double, y: Double, frameWidth: Double, frameHeight: Double)
        -> ChromeHit
    {

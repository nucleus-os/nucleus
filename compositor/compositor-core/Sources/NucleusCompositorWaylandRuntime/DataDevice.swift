...
private final class ActiveWaylandDrag {
    ...
    let initiatingClientKey: WaylandClientID
    var targetSurfaceID: UInt32 = 0
    var targetDevices: [WeakReference<WlDataDevice>] = []
    var targetOffers: [WeakReference<WlDataOffer>] = []

    init(
        source: WlDataSource?,
        origin: WlSurface,
        icon: WlSurface?,
        initiatingClientKey: WaylandClientID
    ) {
        self.source = source
        self.origin = origin
        self.icon = icon
...
@MainActor
@safe final class WlDataDeviceManager {
    ...
    fileprivate func startDrag(
        ...
        _ = dataExchange.startDrag(
            source: sourceHandle,
            origin: originID,
            icon: iconID,
            target: DataClientHandle(
                rawValue: UInt64(initiatingClientKey.opaqueValue)),
            serial: serial)
    }
    ...
    func dragMotion(
        ...
        for box in drag.targetDevices {
            box.value?.protocolResource.sendMotion(
                time: timeMsec, x: x, y: y)
        }
        return true
    }
    ...
    func dropActiveDrag() -> Bool {
        ...
        for box in drag.targetDevices {
            box.value?.protocolResource.sendDrop()
        }
    ...
    func cancelActiveDrag(notifySource: Bool) {
        ...
        if notifySource {
            ...
            if let source = drag.source {
                let handle = source.exchangeHandle
                dataExchange.cancelDrag(source: handle)
            }
        }
        activeDrag = nil
    }
    ...
    fileprivate func finishDrag(from offer: WlDataOffer) {
        guard let drag = activeDrag,
            drag.targetOffers.contains(where: { $0.value === offer })
        else { return }
    ...
    private func enterTarget(
        ...
        for device in targets {
            ...
            if let offer {
                drag.targetOffers.append(WeakReference(offer))
            }
            device.protocolResource.sendEnter(
                serial: serial,
                surface: surfaceResource,
                x: x,
                y: y,
                id: offer?.protocolResource)
        }
    }
    ...
    private func leaveCurrentTarget(_ drag: ActiveWaylandDrag) {
        ...
        for box in drag.targetDevices {
            box.value?.protocolResource.sendLeave()
        }
...
@safe final class WlDataSource {
    ...
    private unowned let dataExchange: DataExchangeService
    ...
    init(
        resource: WaylandResourceHandle<WlDataSourceServer>,
        ...
        let handle = DataSourceHandle(rawValue: dataExchange.allocateHandle())
        dataExchange.sourceCreated(
            handle,
            ownerKind: .wayland,
            client: DataClientHandle(
                rawValue: UInt64(clientKey.opaqueValue)))
    ...
    isolated deinit {
        ...
        let handle = exchangeHandle
        _ = dataExchange.sourceDestroyed(handle)
    }
    ...
    fileprivate func sendTarget(_ mime: String?) {
        resourceHandle.sendTarget(mime_type: mime)
    }

    fileprivate func sendAction(_ action: UInt32) {
        guard resourceHandle.supportsAction else { return }
        resourceHandle.sendAction(
            dnd_action: WlDataDeviceManagerDndAction(rawValue: action))
    }

    fileprivate func sendDropPerformed() {
        guard resourceHandle.supportsDndDropPerformed else { return }
        resourceHandle.sendDndDropPerformed()
    }

    fileprivate func sendDndFinished() {
        guard resourceHandle.supportsDndFinished else { return }
        resourceHandle.sendDndFinished()
    }
...
@safe final class WlDataOffer {
    ...
    init(
        resource: WaylandResourceHandle<WlDataOfferServer>,
...
extension WlDataOffer: WlDataOfferRequests {
    func accept(
        ...
        acceptedMimeType = mime_type.flatMap {
    ...
    func setActions(
        ...
        do {
            let result = try DndActionNegotiation.resolve(
                ...
                destinationActions: dnd_actions.rawValue,
                preferredAction: preferred_action.rawValue)
    ...
    fileprivate func sendSelectionOffer(_ source: (any SelectionSource)?) {
        ...
        _ = resourceHandle.createDataOffer(
            ...
            installed: { offer in
                ...
                resourceHandle.sendSelection(id: offer.protocolResource)
            })
    }
    ...
    fileprivate func makeDragOffer(
        ...
        resourceHandle.createDataOffer(
            ...
            installed: { offer in
                ...
                if offer.protocolResource.supportsSourceActions {
                    offer.protocolResource.sendSourceActions(
                        source_actions: WlDataDeviceManagerDndAction(
                            rawValue: source.effectiveDragActions))
                }
            })
    }

    isolated deinit { manager?.removeDevice(self) }
}
...
extension WlDataDevice: WlDataDeviceRequests {
    ...
    func startDrag(
        ...
        if let icon, !icon.claimDragIconRole() {
            request.postError(.role, message: "drag icon surface already has an incompatible role")
            return
        }

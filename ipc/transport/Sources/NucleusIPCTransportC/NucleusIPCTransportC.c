...
#define NUCLEUS_IPC_MAX_FDS 64

static int nucleus_ipc_set_cloexec(int fd) {
...
static int nucleus_ipc_socket_address(
    const char *path,
...
int nucleus_ipc_listen(const char *path, uint32_t mode) {
    struct sockaddr_un address;
    socklen_t length = 0;
    if (nucleus_ipc_socket_address(path, &address, &length) < 0) return -1;
...
int nucleus_ipc_accept(int listener_fd) {
    ...
    return fd < 0 ? -1 : nucleus_ipc_set_cloexec(fd);
}
...
int nucleus_ipc_send(
    ...
    const int *fds,
    size_t fd_count) {
    if (!bytes || byte_count == 0 || fd_count > NUCLEUS_IPC_MAX_FDS ||
        (fd_count > 0 && !fds)) {
        errno = EINVAL;
        return -1;
    }
    struct iovec iov = {.iov_base = (void *)bytes, .iov_len = byte_count};
    char control[CMSG_SPACE(sizeof(int) * NUCLEUS_IPC_MAX_FDS)];
...
int nucleus_ipc_receive(
    ...
    if (!bytes || byte_capacity == 0 || !output_fd_count ||
        fd_capacity > NUCLEUS_IPC_MAX_FDS || (fd_capacity > 0 && !fds)) {
        errno = EINVAL;
        return -1;
    }
    *output_fd_count = 0;

...
- [x] Enforce product, linkage, functional, and official-capability parity
  before publication.
- [x] Expose and verify the complete workflow through `collider`.
...
## Phase 8: Integrate the contract into `collider`

Expose the complete workflow through the existing top-level command:

- `collider doctor` verifies sources, submodules, host prerequisites,
  component revisions, and product-manifest consistency.
- `collider build` constructs, assembles, validates, and atomically
  publishes the complete toolchain as part of the staged bootstrap graph.
- `collider test` runs the functional toolchain and Nucleus integration
  gates.
...
8. Build and test Nucleus through `collider`.

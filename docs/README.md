...
2. Complete the
   [source identity from Git objects plan](source-identity-from-git-objects-plan.md)
   so source closure identity derives from the object identities Git already
   maintains, and content is read only where Git cannot answer. Capture runs on
   every protected-main verification and every revalidated local invocation, so
   its cost is paid by every phase below it.

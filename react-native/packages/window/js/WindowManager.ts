...
interface NativeWindowManagerSpec extends TurboModule {
   ...
   */
  createSurface(windowId: number, moduleName: string, initialPropsJson: string): Promise<number>;
...
const NativeWindowManager = TurboModuleRegistry.get<NativeWindowManagerSpec>('WindowManager');

function normalizeCreateOptions(options?: WindowCreateOptions): NativeWindowCreateOptions | null {
  if (!options) {
    return null;
  }
  ...
  ? {
      ...
      createSurface(windowId, moduleName, initialProps) {
        ...
        try {
          ...
          throw new Error(`createSurface initialProps must be JSON-serializable: ${String(err)}`);
        }

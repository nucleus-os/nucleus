...
function makeMetroConfig(appRoot) {
  ...
  const monorepoRoot = nucleusRoot
    ? path.resolve(nucleusRoot, '../..')
  ...
  const config = mergeConfig(baseConfig, {
    ...
    resolver: {
      ...
      sourceExts: Array.from(
        new Set([...defaultSourceExts, 'ts', 'tsx', 'nucleus.ts', 'nucleus.tsx']),
      ),
    ...
    serializer: {
      getModulesRunBeforeMainModule: () => [resolveFrom('react-native/setup-env', appRoot)],
    },
  });

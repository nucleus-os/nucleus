...
const surfaceId = await WindowManager.createSurface(windowId, 'MyApp', { initialProps: {} });
...
function App() {
  return (
    <>
      ...
      <Window width={600} height={400} title="Second Window" component={SecondWindow} />
    </>
  );
}
...
function SecondWindow() {
  return (
    <View>
      <Text>This renders in a separate window!</Text>
    </View>
  );
}
```

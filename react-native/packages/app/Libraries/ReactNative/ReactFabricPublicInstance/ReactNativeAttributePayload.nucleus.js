...
function diffProperties(
  ...
  for (const propKey in nextProps) {
    ...
    if (typeof attributeConfig !== 'object') {
      ...
      if (defaultDiffer(prevProp, nextProp)) {
        ...
        (updatePayload || (updatePayload = ({}: { [string]: $FlowFixMe })))[propKey] = nextProp;
      }
    } else if (
      typeof attributeConfig.diff === 'function' ||
      typeof attributeConfig.process === 'function'
        ...
        const nextValue =
          typeof attributeConfig.process === 'function'
            ? // $FlowFixMe[incompatible-use] found when upgrading Flow
              attributeConfig.process(nextProp)
            : nextProp;
        (updatePayload || (updatePayload = ({}: { [string]: $FlowFixMe })))[propKey] = nextValue;
      }
    } else {
  ...
  for (const propKey in prevProps) {
    ...
    if (
      ...
      (updatePayload || (updatePayload = ({}: { [string]: $FlowFixMe })))[propKey] = null;
      if (!removedKeys) {
...
export function create(props: Object, validAttributes: AttributeConfiguration): null | Object {
  const result = addNestedProperty(null, props, validAttributes);

  if (__DEV__ && global.__NUCLEUS_TRACE_ATTRIBUTE_PAYLOAD__ === true) {
    console.log('[AttributePayload] create() result:', result);
  }

...
function createDefaultShimMap({ reactNativeRoot, nucleusRoot }) {
  return {
    ...
    'react-native/Libraries/NativeComponent/ViewConfigIgnore': {
      path: path.join(nucleusRoot, 'Libraries', 'NativeComponent', 'ViewConfigIgnore.nucleus.js'),
    },
    ...
    'react-native/Libraries/ReactNative/AppContainer-dev': {
      path: path.join(nucleusRoot, 'Libraries', 'ReactNative', 'AppContainer-dev.nucleus.js'),
    },
    'react-native/Libraries/Components/TextInput/TextInput': {
      path: path.join(nucleusRoot, 'Libraries', 'Components', 'TextInput', 'TextInput.nucleus.js'),
    },
    ...
    'react-native/Libraries/Modal/Modal': {
      path: path.join(nucleusRoot, 'Libraries', 'Modal', 'Modal.nucleus.js'),
    },
    'react-native/Libraries/Components/Switch/Switch': {
      path: path.join(nucleusRoot, 'Libraries', 'Components', 'Switch', 'Switch.nucleus.tsx'),
    },
  };
}
    ...
    resolveRequest(context, moduleName, platform) {
      ...
      try {
        ...
        if (
          ...
          res.filePath.includes(
            `${path.sep}node_modules${path.sep}.pnpm${path.sep}react-native@`,
          ) &&
          !toPosix(res.filePath).startsWith(toPosix(reactNativeRoot))
        ) {
          const idx = res.filePath.lastIndexOf(
            `${path.sep}node_modules${path.sep}react-native${path.sep}`,
          );
          if (idx !== -1) {

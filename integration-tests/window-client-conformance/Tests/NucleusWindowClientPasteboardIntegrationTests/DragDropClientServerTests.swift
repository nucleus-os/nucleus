...
private func pumpDragFixtureSetup(
    ...
    clients: [NucleusDesktopConnection],
    cycles: Int = 32
) {
    for _ in 0..<cycles {
...
struct DragDropClientServerTests {
    ...
    @safe private final class Peer {
        ...
        let root: View
        let drag: NucleusDesktopDragDropAdapter
        let pasteboard: NucleusDesktopPasteboardAdapter
        ...
        init(
            ...
            guard let dragAdapter = NucleusDesktopDragDropAdapter(
                client: client,
                seat: seat,
                destinationResolver: {
            ...
            drag = dragAdapter
            pasteboard = try #require(NucleusDesktopPasteboardAdapter(
                client: client,
                seat: seat))
            pumpDragFixtureSetup(runtime, clients: [client])
        }
    ...
    func cancellationAndSurfaceTeardownFinishEachSessionOnce() async throws {
        ...
        let unauthorizedSession = source.drag.startDrag(
            from: sourceView,
        ...
        let authorizedSession = source.drag.startDrag(
            from: sourceView,
    ...
    private func processReady(
        _ drag: NucleusDesktopDragDropAdapter
    ) {
    ...
    private func processReady(
        _ pasteboard: NucleusDesktopPasteboardAdapter
    ) {
    ...
    private func process(
        descriptor: NucleusDesktopTransferPollDescriptor,
        apply: (
            _ token: UInt64,
            _ result: NucleusWindowClientPollResult,
        ...
        apply(
            descriptor.token,
            NucleusWindowClientPollResult(revents: polled.revents),
            1)
    }

...
@MainActor
@Suite(.uiContext) struct NucleusDesktopTextInputTests {
    // MARK: - Content type

    @Test func contentTypesMapOntoProtocolPurposes() {
        // Values from the protocol's own enumeration.
        #expect(NucleusDesktopTextInput.contentPurpose(.normal) == 0)
        #expect(NucleusDesktopTextInput.contentPurpose(.number) == 3)
        #expect(NucleusDesktopTextInput.contentPurpose(.phone) == 4)
        #expect(NucleusDesktopTextInput.contentPurpose(.url) == 5)
        #expect(NucleusDesktopTextInput.contentPurpose(.email) == 6)
        #expect(NucleusDesktopTextInput.contentPurpose(.name) == 7)
        #expect(NucleusDesktopTextInput.contentPurpose(.password) == 8)
        #expect(NucleusDesktopTextInput.contentPurpose(.pin) == 9)
    }

    @Test func hintsMapOntoProtocolFlags() {
        #expect(NucleusDesktopTextInput.contentHint([]) == 0)

        let spellcheck = NucleusDesktopTextInput.contentHint([.spellcheck])
        #expect(spellcheck & 0x2 != 0)

        let multiline = NucleusDesktopTextInput.contentHint([.multiline])
    ...
    @Test func sensitiveDataSetsBothProtectionFlags() {
        let hint = NucleusDesktopTextInput.contentHint([.sensitiveData])
    ...
    @Test func aSecureFieldReachesTheWireAsAProtectedPassword() {
        ...
        #expect(NucleusDesktopTextInput.contentPurpose(field.textInputContentType) == 8)
        let hint = NucleusDesktopTextInput.contentHint(field.textInputHints)
        #expect(hint & 0x80 != 0)
        #expect(hint & 0x40 != 0)
    ...
    @Test func preeditCursorConvertsFromUTF8Bytes() {
        // "a" + 4-byte emoji + "b"
        let text = "a😀b"
        #expect(NucleusDesktopTextInput.utf16Offset(in: text, forUTF8: 0) == 0)
        #expect(NucleusDesktopTextInput.utf16Offset(in: text, forUTF8: 1) == 1)
        #expect(NucleusDesktopTextInput.utf16Offset(in: text, forUTF8: 5) == 3)
        #expect(NucleusDesktopTextInput.utf16Offset(in: text, forUTF8: 6) == 4)
    }

    @Test func aPreeditSelectionSpansTheGivenRange() throws {
        let range = try #require(NucleusDesktopTextInput.preeditSelection("にほんご", begin: 3, end: 6))
    ...
    @Test func aHiddenPreeditCursorYieldsNoSelection() {
        #expect(NucleusDesktopTextInput.preeditSelection("にほん", begin: -1, end: -1) == nil)
    }

    @Test func aReversedPreeditCursorIsNormalized() throws {
        let range = try #require(NucleusDesktopTextInput.preeditSelection("abcd", begin: 3, end: 1))
        #expect(range == 1..<3)
    }

    @Test func anOutOfRangePreeditCursorClampsToTheEnd() {
        #expect(NucleusDesktopTextInput.utf16Offset(in: "ab", forUTF8: 99) == 2)
    }

    @Test func aMidScalarOffsetClampsToThePreviousBoundary() {
        #expect(NucleusDesktopTextInput.utf16Offset(
            in: "a😀b",
            forUTF8: 3) == 1)
    }
    ...
    @Test func surroundingTextIsBoundedWithoutSplittingScalars() throws {
        ...
        let context = try #require(
            NucleusDesktopTextInput.boundedSurroundingContext(
    ...
    @Test func malformedSurroundingOffsetsAndGeometryAreRejected() {
        #expect(NucleusDesktopTextInput.boundedSurroundingContext(
            TextInputSurroundingContext(
                text: "a😀b",
                cursorByteOffset: 3,
                anchorByteOffset: 3)) == nil)
        #expect(NucleusDesktopTextInput.wireRectangle(Rect(
            x: .nan,
            y: 0,
            width: 1,
            height: 1)) == nil)
    ...
    @Test func multilineTextViewUsesTheSharedShellInputContract() {
        ...
        #expect(
            NucleusDesktopTextInput.contentHint(editor.textInputHints) & 0x200
                != 0)
